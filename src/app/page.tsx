...
function CovTab({cov,onRefresh}:{cov:any;onRefresh?:()=>void}){
  ...
  return <><div style={{display:'flex',justifyContent:'flex-end',marginBottom:6,gap:4}}>{onRefresh&&<button className="tc-btn" onClick={onRefresh} style={{fontSize:'.66rem',padding:'3px 8px'}}>↻ Refresh Coverage</button>}</div><div className="kpi-grid"><div className="kpi"><div className="kpi-label">Total Assets</div><div className="kpi-val">{cov.totalDevices?.toLocaleString()}</div></div><div className="kpi"><div className="kpi-label">Agent Coverage</div><div className="kpi-val" style={{color:(cov.agentCoverage||0)>=90?'var(--green)':(cov.agentCoverage||0)>=70?'var(--amber)':'var(--red)'}}>{cov.agentCoverage||0}%</div></div><div className="kpi"><div className="kpi-label">Stale Assets</div><div className="kpi-val" style={{color:(cov.staleCount||0)>0?'var(--amber)':'var(--green)'}}>{cov.staleCount||0}</div><div className="kpi-sub">Not seen in 14+ days</div></div>{toolEntries.map(([k,v]:any)=>(<div key={k} className="kpi"><div className="kpi-label">{k}</div><div className="kpi-val" style={{color:(v.offline||0)>10?'var(--amber)':'var(--green)'}}>{v.installed?.toLocaleString()}</div><div className="kpi-sub">{v.healthy||0} healthy · {v.degraded||0} stale</div></div>))}</div>{osData.length>0&&<div className="panel"><div className="panel-hd"><h3>💻 OS Breakdown</h3><span className="count">{osData.length} types</span></div><div style={{padding:14}}>{osData.map((o:any)=>(<div key={o.os} style={{display:'flex',alignItems:'center',gap:8,marginBottom:7}}><span style={{minWidth:100,fontSize:'.72rem',fontWeight:600,color:'var(--t1)'}}>{o.os}</span><div className="bar-wrap" style={{flex:1}}><div className="bar-track"><div className="bar-fill" style={{width:`${o.pct}%`,background:o.os.includes('Server 2008')||o.os.includes('Server 2012')?'var(--red)':'var(--accent)'}}/></div></div><span className="mono" style={{fontSize:'.68rem',minWidth:50,textAlign:'right'}}>{o.count.toLocaleString()} ({o.pct}%)</span></div>))}</div></div>}<div className="panel"><div className="panel-hd"><h3>⚠ Coverage Gaps</h3><span className="count">{gaps.length}{gaps.length>=30?'+':''}</span></div>{gaps.length===0?<div style={{padding:20,textAlign:'center',color:'var(--t3)',fontSize:'.78rem'}}>No coverage gaps detected</div>:<div className="tbl-wrap" style={{maxHeight:400}}><table className="tbl"><thead><tr><th>Host</th><th className="desk-only">IP</th><th className="desk-only">OS</th><th>Missing</th><th className="desk-only">Reason</th><th className="desk-only">Last Seen</th></tr></thead><tbody>{gaps.map((g:any,i:number)=>(<tr key={i}><td className="device" style={{fontWeight:600}}>{g.hostname}</td><td className="desk-only mono" style={{fontSize:'.7rem'}}>{g.ip||'—'}</td><td className="desk-only" style={{fontSize:'.7rem'}}>{g.os}</td><td>{g.missing.map((m:string)=><span key={m} className="src" style={{marginRight:3}}>{m}</span>)}</td><td className="muted desk-only" style={{fontSize:'.72rem'}}>{g.reason}</td><td className="desk-only ts">{g.lastSeen?ago(g.lastSeen):'Never'}</td></tr>))}</tbody></table></div>}</div></>;
}
...
.clickable-row{transition:all .12s}.clickable-row:hover{background:var(--accent-s)!important}

...
class ErrorBoundary extends React.Component<{children:React.ReactNode;fallback?:string},{error:any}>{
  constructor(p:any){super(p);this.state={error:null}}
  static getDerivedStateFromError(e:any){return{error:e}}
  render(){if(this.state.error)return <div style={{padding:40,color:'#f0384a',background:'#0b0e16',fontFamily:'monospace',fontSize:'.8rem'}}><h2>Dashboard Error</h2><p style={{marginTop:8}}>{String(this.state.error)}</p><button onClick={()=>this.setState({error:null})} style={{marginTop:12,padding:'6px 12px',background:'#4f8fff',color:'#fff',border:'none',borderRadius:6,cursor:'pointer'}}>Retry</button></div>;return this.props.children}
}
...
function ThreatGlobe({size=200}:{size?:number}){
  ...
  useEffect(()=>{
    ...
    function draw(){
      ...
      // Globe
      ctx.beginPath();ctx.arc(cx,cy,r,0,Math.PI*2);ctx.fillStyle='#10141e';ctx.fill();ctx.strokeStyle='#1a2030';ctx.lineWidth=1;ctx.stroke();
      // Grid lines
      ctx.strokeStyle='#252d4060';ctx.lineWidth=.5;
      ...
      // Labels
      const labels=[{lat:51.5,lon:-0.1,name:'UK'},{lat:40.7,lon:-74,name:'US'},{lat:55.7,lon:37.6,name:'RU'},{lat:31.2,lon:121.5,name:'CN'},{lat:35.7,lon:139.7,name:'JP'},{lat:-33.9,lon:18.4,name:'ZA'}];
      labels.forEach(l=>{const[lx,ly,lz]=project(l.lat,l.lon);if(lz>0.1){ctx.fillStyle='#8893ab';ctx.font='7px monospace';ctx.textAlign='center';ctx.fillText(l.name,lx,ly-6)}});
      rot+=0.15;
...
function CollabBar(){
  ...
  if(!data||!data.predictions)return <div className="panel"><div className="panel-hd"><h3>🔮 Predictions</h3></div><div style={{padding:16,textAlign:'center',color:'var(--t3)',fontSize:'.78rem'}}>Loading predictions...</div></div>;
...
function VulnDetail({vuln,onClose}:{vuln:any;onClose:()=>void}){
  ...
  useEffect(()=>{if(!vuln)return;setLoading(true);fetch('/api/tenable/hosts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pluginId:String(vuln.id).replace('CVE-','')})}).then(r=>r.ok?r.json():{hosts:[]}).then(d=>{setHosts(d.hosts||[]);if(d.error)setError(d.error);setLoading(false)}).catch(e=>{setError(String(e));setLoading(false)})},[vuln]);
  if(!vuln)return null;
  return <div className="drawer-overlay" onClick={onClose}><div className="drawer" onClick={e=>e.stopPropagation()}><div className="guide-hd"><div><h2 style={{fontSize:'.9rem',fontWeight:800,fontFamily:'var(--fm)',color:'var(--red)'}}>{vuln.id}</h2><p style={{fontSize:'.78rem',fontWeight:600,color:'var(--t1)',marginTop:2}}>{vuln.name}</p><div style={{display:'flex',gap:6,marginTop:6}}><span className="sev sev-critical" style={{fontFamily:'var(--fm)'}}>CVSS {vuln.cvss}</span><span style={{fontSize:'.62rem',color:'var(--t3)',fontFamily:'var(--fm)',background:'var(--bg3)',padding:'1px 6px',borderRadius:4}}>{vuln.hosts} hosts</span></div></div><button className="modal-close" onClick={onClose}>✕</button></div><div className="guide-body">{loading?<div style={{textAlign:'center',padding:24}}><span className="spin" style={{display:'inline-block'}}/>Loading affected hosts...</div>:<>{error&&<div style={{fontSize:'.68rem',color:'var(--amber)',marginBottom:8,padding:'4px 8px',background:'var(--ambers)',borderRadius:4}}>{error}</div>}<div style={{fontSize:'.68rem',color:'var(--t3)',marginBottom:8}}>{hosts.length} affected host{hosts.length!==1?'s':''}</div>{hosts.map((h:any,i:number)=>(<div key={i} className="device-alert-card"><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontFamily:'var(--fm)',fontSize:'.8rem',fontWeight:700,color:'var(--t1)'}}>{h.hostname}</div><div style={{fontSize:'.68rem',color:'var(--t3)',fontFamily:'var(--fm)'}}>{h.ip}</div></div><span style={{fontSize:'.58rem',color:h.state==='open'?'var(--red)':'var(--green)',background:h.state==='open'?'var(--reds)':'var(--greens)',padding:'1px 6px',borderRadius:4,fontWeight:600,textTransform:'uppercase'}}>{h.state}</span></div>{h.os&&<div style={{fontSize:'.65rem',color:'var(--t2)',marginTop:4}}>{h.os}</div>}{h.lastSeen&&<div style={{fontSize:'.58rem',color:'var(--t3)',fontFamily:'var(--fm)',marginTop:2}}>Last seen: {ago(h.lastSeen)}</div>}</div>))}{hosts.length===0&&!loading&&<div style={{textAlign:'center',padding:20,color:'var(--t3)'}}>No host details available</div>}{vuln.solution&&<div style={{marginTop:12,padding:10,background:'var(--greens)',border:'1px solid var(--green)',borderRadius:'var(--r)'}}><div style={{fontSize:'.65rem',fontWeight:700,color:'var(--green)',marginBottom:3}}>REMEDIATION</div><div style={{fontSize:'.74rem',color:'var(--t1)'}}>{vuln.solution}</div></div>}{vuln.exploitable&&<div style={{marginTop:8,padding:8,background:'var(--reds)',border:'1px solid var(--red)',borderRadius:'var(--r)',fontSize:'.72rem',color:'var(--red)',fontWeight:600}}>⚠ Known exploit available</div>}{vuln.slaBreached&&<div style={{marginTop:8,padding:8,background:'var(--reds)',border:'1px solid var(--red)',borderRadius:'var(--r)',fontSize:'.72rem',color:'var(--red)',fontWeight:600}}>⏰ SLA BREACHED — open {vuln.ageDays} days</div>}</>}</div></div></div>;
}


function Safe({children}:{children:React.ReactNode}){return <ErrorBoundary>{children}</ErrorBoundary>}
/* ═══ MAIN ═══ */
export default function Dashboard(){
  ...
  useEffect(()=>{
    function onKey(e:KeyboardEvent){
      ...
      if(e.key==='Escape'){setGuideOpen(false);setCmdOpen(false);setIocOpen(false);setAiAlert(null);setDeviceDrill(null);setNotesAlert(null);setVulnDetail(null)}
    }
  ...
  return(<ErrorBoundary><style dangerouslySetInnerHTML={{__html:CSS}}/><div className={`shell ${hasCrit?'crit-flash':''}`}><div className="topbar"><div className="logo"><div className="logo-icon">S</div><span>Sec</span>Ops</div><div className="tabs desk-only">{tabs.map(t=>(<button key={t.k} className={`tab ${tab===t.k?'active':''}`} onClick={()=>setTab(t.k)}>{t.i} {t.l}{t.badge&&t.badge>0?<span className="tab-badge">{t.badge}</span>:null}</button>))}</div><button className="mob-menu mob-only" onClick={()=>setMobileNav(!mobileNav)}>☰</button><div className="topbar-right"><button className="search-trigger" onClick={()=>setIocOpen(true)}>🔍 <span className="desk-only">IOC Search</span></button><span className="clock desk-only">{clock}</span><div className="live-dot"/>{demo&&<div className="demo-pill desk-only">DEMO</div>}<button className="theme-btn desk-only" onClick={toggleFullscreen} title="Fullscreen">{isFullscreen?'⊡':'⛶'}</button><button className="theme-btn" onClick={()=>setTheme(t=>t==='dark'?'light':'dark')}>{theme==='dark'?'☀':'🌙'}</button><button className="guide-btn" onClick={()=>setGuideOpen(true)} title="User Guide">📖</button><button className="refresh-btn desk-only" onClick={refresh}>↻</button></div></div>{mobileNav&&<div className="mob-nav">{tabs.map(t=>(<button key={t.k} className={`mnav-btn ${tab===t.k?'active':''}`} onClick={()=>{setTab(t.k);setMobileNav(false)}}>{t.i} {t.l}</button>))}</div>}<div className="main">{iocOpen&&<IOCSearch open={iocOpen} onClose={()=>setIocOpen(false)}/>}{aiAlert&&<AICopilot alert={aiAlert} onClose={()=>setAiAlert(null)}/>}<UserGuide open={guideOpen} onClose={()=>setGuideOpen(false)}/><CmdPalette open={cmdOpen} onClose={()=>setCmdOpen(false)} onSelect={cmdSelect}/><DeviceDrawer hostname={deviceDrill} alerts={alerts} onClose={()=>setDeviceDrill(null)}/><AlertNotes alertId={notesAlert} onClose={()=>setNotesAlert(null)}/>{vulnDetail&&<VulnDetail vuln={vulnDetail} onClose={()=>setVulnDetail(null)}/>}{loading?<div className="loading"><span className="spin"/>Loading...</div>:tab==='overview'?<Ov m={m} cov={cov} alerts={alerts} zsc={zsc} sparks={sparks} enabledTools={enabledTools} onAskAI={(a:Al)=>setAiAlert(a)} onRefresh={refresh}/>:tab==='alerts'?<Als alerts={alerts} onAskAI={(a:Al)=>setAiAlert(a)} onDeviceDrill={(h:string)=>setDeviceDrill(h)} onNotes={(id:string)=>setNotesAlert(id)}/>:tab==='coverage'?<CovTab cov={cov} onRefresh={refresh}/>:tab==='vulns'?<Vul onVulnClick={(v:any)=>setVulnDetail(v)}/>:<ToolsManager toolsData={toolsData} onRefresh={refresh}/>}</div><ThreatTicker/></div></ErrorBoundary>);
}
...
/* ═══ HERO SECTION (needs metrics for severity ring) ═══ */
function HeroSection({m,sparks,enabledTools}:any){
  return <div className="hero-grid"><div className="panel hero-panel"><div className="panel-hd"><h3>🌍 Threat Globe</h3></div><div style={{padding:'6px 0'}}><ThreatGlobe size={160}/></div><div style={{textAlign:'center',fontSize:'.55rem',color:'var(--t3)',paddingBottom:6}}>Live attack arcs</div></div><div className="panel hero-panel"><div className="panel-hd"><h3>🎯 Severity</h3></div><div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:14,padding:'14px 10px'}}><SevRing c={m?.alertsLast24h?.critical||0} h={m?.alertsLast24h?.high||0} m={m?.alertsLast24h?.medium||0} l={m?.alertsLast24h?.low||0} size={90}/><div style={{fontSize:'.7rem',lineHeight:2}}><div><span className="sev sev-critical">{m?.alertsLast24h?.critical||0}</span> Crit</div><div><span className="sev sev-high">{m?.alertsLast24h?.high||0}</span> High</div><div><span className="sev sev-medium">{m?.alertsLast24h?.medium||0}</span> Med</div><div><span className="sev sev-low">{m?.alertsLast24h?.low||0}</span> Low</div></div></div></div><div className="panel hero-panel"><div className="panel-hd"><h3>📈 Hourly Alerts</h3></div><div style={{padding:'14px 10px',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}><HourlyChart data={sparks.hourly} w={200} h={60}/><div style={{display:'flex',justifyContent:'space-between',width:200,fontSize:'.52rem',color:'var(--t3)',fontFamily:'var(--fm)'}}><span>24h ago</span><span>12h</span><span>Now</span></div></div></div><div className="panel hero-panel"><div className="panel-hd"><h3>🔌 Connected</h3></div><div style={{padding:'10px',display:'flex',flexWrap:'wrap',gap:6,justifyContent:'center'}}>{enabledTools.map((t:any)=>(<div key={t.id} className="tool-chip" style={{borderColor:t.color+'33',color:t.color}}><span>{t.icon}</span>{t.shortName}</div>))}{enabledTools.length===0&&<div style={{fontSize:'.72rem',color:'var(--t3)',padding:12}}>No tools — go to Tools tab</div>}</div></div></div>;
}

/* ═══ OVERVIEW ═══ */
function Ov({m,cov,alerts,zsc,sparks,enabledTools,onAskAI,onRefresh}:any){
  return <><div style={{display:'flex',justifyContent:'flex-end',gap:6,marginBottom:8}}><button className="tc-btn" onClick={onRefresh} style={{fontSize:'.66rem',padding:'3px 8px'}}>↻ Refresh All</button><ExecSummary metrics={m} alerts={alerts} coverage={cov}/></div><KpiSection m={m} cov={cov} zsc={zsc} sparks={sparks} enabledTools={enabledTools} alerts={alerts}/><HeroSection m={m} sparks={sparks} enabledTools={enabledTools}/><div className="g23"><div><div className="panel"><div className="panel-hd"><h3>⚡ Critical &amp; High</h3><span className="count">{alerts.filter((a:Al)=>a.severity==='critical'||a.severity==='high').length}</span></div><div className="tbl-wrap" style={{maxHeight:280}}><table className="tbl"><thead><tr><th>Alert</th><th>Source</th><th>Sev</th><th>Time</th></tr></thead><tbody>{alerts.filter((a:Al)=>a.severity==='critical'||a.severity==='high').sort((a,b)=>SO[a.severity]-SO[b.severity]).slice(0,8).map((a:Al)=>(<tr key={a.id}><td style={{fontWeight:600}}>{a.title}</td><td><span className={`src ${sc(a.source)}`}>{a.source}</span></td><td><span className={`sev sev-${a.severity}`}>{a.severity}</span></td><td className="ts">{ago(a.timestamp)}</td></tr>))}</tbody></table></div></div>{m&&m.topSources&&<div className="panel"><div className="panel-hd"><h3>📊 Sources</h3></div><div style={{padding:14}}>{m.topSources.map((s:any)=>(<div key={s.source} style={{display:'flex',alignItems:'center',gap:8,marginBottom:7}}><span className={`src ${sc(s.source)}`} style={{minWidth:80}}>{s.source}</span><div className="bar-wrap" style={{flex:1}}><div className="bar-track"><div className="bar-fill" style={{width:`${s.pct}%`,background:'var(--accent)'}}/></div></div><span className="mono" style={{fontSize:'.72rem'}}>{s.count}</span></div>))}</div></div>}</div><div className="panel" style={{overflow:'hidden'}}><div className="panel-hd"><h3>🕐 Timeline</h3></div><div style={{overflowY:'auto',maxHeight:460,padding:'6px 14px'}}>{TL.map(t=>(<div key={t.id} className="tl-item"><div className="tl-icon">{t.icon}</div><div className="tl-body"><div className="tl-title">{t.title}</div><div className="tl-meta"><span className={`src ${sc(t.source)}`}>{t.source}</span><span className="ts">{ago(t.time)}</span></div></div></div>))}</div></div></div><NLQuery/><div className="g2r"><AttackChainGraph/><PredictionsPanel/></div><MitreHeatmap alerts={alerts}/><TrendCharts/></>;
}
...
function Vul({onVulnClick}:{onVulnClick?:(v:any)=>void}){
  ...
  const s=d.summary||{};const allV=d.allVulns||d.topCritical||[];const sla=d.slaStats||{};const topH=d.topHosts||[];
  const sevMap:Record<string,number>={critical:4,high:3,medium:2,low:1};
  const filtered=allV.filter((v:any)=>sevFilter==='all'||v.sevLabel===sevFilter||(sevFilter==='sla'&&v.slaBreached)).filter((v:any)=>!search||v.name?.toLowerCase().includes(search.toLowerCase())||(v.cve&&v.cve.toLowerCase().includes(search.toLowerCase()))||String(v.id).includes(search));
  function exportCSV(){const rows=filtered.map((v:any)=>({plugin_id:v.id,cve:v.cve||'',name:v.name,severity:v.sevLabel,cvss:v.cvss,hosts:v.hosts,age_days:v.ageDays,exploitable:v.exploitable?'Yes':'No',sla_breached:v.slaBreached?'Yes':'No',solution:v.solution||''}));if(!rows.length)return;const keys=Object.keys(rows[0]);const csv=[keys.join(','),...rows.map((r:any)=>keys.map(k=>JSON.stringify(r[k]??'')).join(','))].join('\n');const b=new Blob([csv],{type:'text/csv'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='vulnerabilities.csv';a.click();URL.revokeObjectURL(u)}
  return <><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:6,marginBottom:8}}><div className="pills"><button className={`pill ${sevFilter==='all'?'on':''}`} onClick={()=>setSevFilter('all')}>All ({s.total||allV.length})</button><button className={`pill ${sevFilter==='critical'?'on':''}`} onClick={()=>setSevFilter('critical')} style={{color:sevFilter==='critical'?'var(--red)':''}}>Critical ({s.critical||0})</button><button className={`pill ${sevFilter==='high'?'on':''}`} onClick={()=>setSevFilter('high')} style={{color:sevFilter==='high'?'#f97316':''}}>High ({s.high||0})</button><button className={`pill ${sevFilter==='medium'?'on':''}`} onClick={()=>setSevFilter('medium')}>Medium ({s.medium||0})</button><button className={`pill ${sevFilter==='low'?'on':''}`} onClick={()=>setSevFilter('low')}>Low ({s.low||0})</button>{(sla.breached||0)>0&&<button className={`pill ${sevFilter==='sla'?'on':''}`} onClick={()=>setSevFilter('sla')} style={{color:'var(--red)'}}>⚠ SLA Breach ({sla.breached})</button>}</div><div style={{display:'flex',gap:4}}><input className="field-input" placeholder="Search CVE, plugin name..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:200,fontSize:'.72rem',padding:'4px 8px'}}/><button className={`tc-btn ${view==='vulns'?'tc-btn-primary':''}`} onClick={()=>setView('vulns')} style={{fontSize:'.62rem',padding:'3px 6px'}}>Vulns</button><button className={`tc-btn ${view==='hosts'?'tc-btn-primary':''}`} onClick={()=>setView('hosts')} style={{fontSize:'.62rem',padding:'3px 6px'}}>Risky Hosts</button><button className="tc-btn" onClick={exportCSV} style={{fontSize:'.62rem',padding:'3px 6px'}}>📥 CSV</button><button className="tc-btn" onClick={reload} disabled={loading} style={{fontSize:'.62rem',padding:'3px 6px'}}>{loading?'...':'↻'}</button></div></div><div className="kpi-grid"><div className="kpi"><div className="kpi-label">Total Vulns</div><div className="kpi-val">{s.total||allV.length}</div></div><div className="kpi"><div className="kpi-label">Critical</div><div className="kpi-val" style={{color:'var(--red)'}}>{s.critical||0}</div></div><div className="kpi"><div className="kpi-label">Assets</div><div className="kpi-val">{d.assetCounts?.total||0}</div></div><div className="kpi"><div className="kpi-label">SLA Breached</div><div className="kpi-val" style={{color:(sla.breached||0)>0?'var(--red)':'var(--green)'}}>{sla.breached||0}</div><div className="kpi-sub">{sla.critOverdue||0} crit overdue · {sla.highOverdue||0} high overdue</div></div></div>{view==='hosts'&&topH.length>0?<div className="panel"><div className="panel-hd"><h3>🔥 Riskiest Hosts</h3><span className="count">{topH.length}</span></div><div className="tbl-wrap"><table className="tbl"><thead><tr><th>Host</th><th>IP</th><th>OS</th><th>Exposure</th><th>ACR</th><th className="desk-only">Agent</th><th className="desk-only">Last Seen</th></tr></thead><tbody>{topH.map((h:any,i:number)=>(<tr key={i}><td className="device" style={{fontWeight:700}}>{h.hostname}</td><td className="mono" style={{fontSize:'.7rem'}}>{h.ip}</td><td style={{fontSize:'.7rem',maxWidth:180}}>{h.os}</td><td><span style={{fontFamily:'var(--fm)',fontWeight:700,color:(h.exposureScore||0)>700?'var(--red)':(h.exposureScore||0)>400?'#f97316':'var(--green)'}}>{h.exposureScore||'—'}</span></td><td className="mono">{h.acrScore||'—'}</td><td className="desk-only">{h.hasAgent?<span style={{color:'var(--green)',fontSize:'.6rem'}}>✓</span>:<span style={{color:'var(--red)',fontSize:'.6rem'}}>✗</span>}</td><td className="desk-only ts">{h.lastSeen?ago(h.lastSeen):'—'}</td></tr>))}</tbody></table></div></div>:<div className="panel"><div className="panel-hd"><h3>🔴 Vulnerabilities</h3><span className="count">{filtered.length}</span></div><div className="tbl-wrap" style={{maxHeight:'calc(100vh - 280px)'}}><table className="tbl"><thead><tr><th>ID</th><th>Name</th><th>Sev</th><th>CVSS</th><th>Hosts</th><th className="desk-only">Age</th><th className="desk-only">Exploit</th><th className="desk-only">SLA</th></tr></thead><tbody>{filtered.slice(0,100).map((v:any)=>(<tr key={v.id} onClick={()=>onVulnClick?.({id:v.cve||('PID-'+v.id),name:v.name,cvss:v.cvss,hosts:v.hosts,solution:v.solution,synopsis:v.synopsis,exploitable:v.exploitable,ageDays:v.ageDays,slaBreached:v.slaBreached})} style={{cursor:'pointer'}}><td className="mono" style={{fontWeight:700,color:v.severity===4?'var(--red)':v.severity===3?'#f97316':'var(--t2)',fontSize:'.68rem'}}>{v.cve||v.id}</td><td style={{fontWeight:600,maxWidth:300,fontSize:'.74rem'}}>{v.name}</td><td><span className={`sev sev-${v.sevLabel}`}>{v.sevLabel}</span></td><td className="mono" style={{fontWeight:700}}>{v.cvss}</td><td className="mono">{v.hosts}</td><td className="desk-only">{v.ageDays>0?<span style={{color:(v.ageDays||0)>90?'var(--red)':(v.ageDays||0)>30?'#f97316':'var(--t2)',fontFamily:'var(--fm)',fontSize:'.68rem'}}>{v.ageDays}d</span>:'—'}</td><td className="desk-only">{v.exploitable?<span style={{color:'var(--red)',fontSize:'.6rem',fontWeight:700,background:'var(--reds)',padding:'1px 5px',borderRadius:3}}>EXPLOIT</span>:<span className="muted">—</span>}</td><td className="desk-only">{v.slaBreached?<span style={{color:'var(--red)',fontSize:'.6rem',fontWeight:700,background:'var(--reds)',padding:'1px 5px',borderRadius:3}}>BREACH</span>:v.ageDays>0?<span style={{fontSize:'.6rem',color:'var(--green)',fontFamily:'var(--fm)'}}>{Math.max(0,v.slaDays-v.ageDays)}d left</span>:'—'}</td></tr>))}</tbody></table></div></div>}<div className="g2r"><div className="panel"><div className="panel-hd"><h3>Asset Risk</h3></div><div style={{padding:16,display:'flex',justifyContent:'space-around',alignItems:'center',flexWrap:'wrap',gap:12}}><Donut val={d.assetCounts?.withCritical||0} max={d.assetCounts?.total||1} color="var(--red)" label="Critical"/><Donut val={d.assetCounts?.withHigh||0} max={d.assetCounts?.total||1} color="#f97316" label="High"/><Donut val={d.assetCounts?.scanned||0} max={d.assetCounts?.total||1} color="var(--green)" label="Scanned"/></div></div><div className="panel"><div className="panel-hd"><h3>Severity Ring</h3></div><div style={{display:'flex',justifyContent:'center',padding:16}}><SevRing c={s.critical||0} h={s.high||0} m={s.medium||0} l={s.low||0} size={110}/></div></div></div></>;
}

/* ═══ TOOLS MANAGER ═══ */
function ToolsManager({toolsData,onRefresh}:{toolsData:any;onRefresh:()=>void}){
  const[selectedTool,setSelectedTool]=useState<string|null>(null);
  const[testResult,setTestResult]=useState<any>(null);const[testing,setTesting]=useState('');

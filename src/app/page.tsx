...
function Ov({m,cov,alerts,zsc,sparks,enabledTools,onAskAI}:any){
  return(<>
    ...
    <KpiSection m={m} cov={cov} zsc={zsc} sparks={sparks} enabledTools={enabledTools} alerts={alerts}/>
    <HeroSection m={m} sparks={sparks} enabledTools={enabledTools}/>
    <div className="g23"><div><div className="panel"><div className="panel-hd"><h3>⚡ Critical & High</h3><span className="count">{alerts.filter((a:Al)=>a.severity==='critical'||a.severity==='high').length}</span></div><div className="tbl-wrap" style={{maxHeight:280}}><table className="tbl"><thead><tr><th>Alert</th><th>Source</th><th>Sev</th><th>Time</th></tr></thead><tbody>{alerts.filter((a:Al)=>a.severity==='critical'||a.severity==='high').sort((a,b)=>SO[a.severity]-SO[b.severity]).slice(0,8).map((a:Al)=>(<tr key={a.id}><td style={{fontWeight:600}}>{a.title}{a.device&&<><br/><span className="device">{a.device}</span></>}</td><td><span className={`src ${sc(a.source)}`}>{a.source}</span></td><td><span className={`sev sev-${a.severity}`}>{a.severity}</span></td><td className="ts">{ago(a.timestamp)}</td></tr>))}</tbody></table></div></div>{m&&<div className="panel"><div className="panel-hd"><h3>📊 Sources</h3></div><div style={{padding:14}}>{m.topSources?.map((s:any)=>(<div key={s.source} style={{display:'flex',alignItems:'center',gap:8,marginBottom:7}}><span className={`src ${sc(s.source)}`} style={{minWidth:80,justifyContent:'center'}}>{s.source}</span><div className="bar-wrap" style={{flex:1}}><div className="bar-track"><div className="bar-fill" style={{width:`${s.pct}%`,background:'var(--accent)'}}/></div></div><span className="mono" style={{fontSize:'.72rem',minWidth:24,textAlign:'right'}}>{s.count}</span></div>)}</div></div>}</div><div className="panel" style={{overflow:'hidden'}}><div className="panel-hd"><h3>🕐 Timeline</h3></div><div style={{overflowY:'auto',maxHeight:460,padding:'6px 14px'}}>{TL.map(t=>(<div key={t.id} className="tl-item"><div className="tl-icon">{t.icon}</div><div className="tl-body"><div className="tl-title">{t.title}</div><div className="tl-meta"><span className={`src ${sc(t.source)}`}>{t.source}</span><span className="ts">{ago(t.time)}</span></div></div></div>))}</div></div></div>
    <NLQuery/>

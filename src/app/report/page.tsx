...
export default function Report() {
  ...
  const gradeColor = score >= 75 ? '#34e8a5' : score >= 50 ? '#ffb340' : '#ff4466';
  ...
  return (<><style dangerouslySetInnerHTML={{ __html: CSS }} /><div className="rpt">
    ...
    <div className="rpt-page">
      ...
      <div className="rpt-score-row"><div className="rpt-score-box" style={{ borderColor: gradeColor }}><div className="rpt-score-val" style={{ color: gradeColor }}>{score}</div><div className="rpt-score-grade" style={{ color: gradeColor }}>{grade}</div><div className="rpt-score-label">Posture Score</div></div><div className="rpt-score-details">{(d.posture?.factors || []).map((f: any) => (<div key={f.name} className="rpt-factor"><div className="rpt-factor-name">{f.name}</div><div className="rpt-factor-bar"><div className="rpt-factor-fill" style={{ width: f.score + '%', background: f.score >= 70 ? '#34e8a5' : f.score >= 40 ? '#ffb340' : '#ff4466' }} /></div><div className="rpt-factor-val">{f.score}%</div></div>))}</div></div>

      <div className="rpt-section"><h2>Executive Summary</h2><div className="rpt-kpis"><div className="rpt-kpi"><div className="rpt-kpi-val">{d.alerts.length}</div><div className="rpt-kpi-label">Total Alerts</div></div><div className="rpt-kpi"><div className="rpt-kpi-val" style={{ color: '#ff4466' }}>{sev.critical}</div><div className="rpt-kpi-label">Critical</div></div><div className="rpt-kpi"><div className="rpt-kpi-val" style={{ color: '#f97316' }}>{sev.high}</div><div className="rpt-kpi-label">High</div></div><div className="rpt-kpi"><div className="rpt-kpi-val">{d.coverage?.totalDevices?.toLocaleString() || 0}</div><div className="rpt-kpi-label">Assets</div></div><div className="rpt-kpi"><div className="rpt-kpi-val">{d.coverage?.agentCoverage || 0}%</div><div className="rpt-kpi-label">Coverage</div></div><div className="rpt-kpi"><div className="rpt-kpi-val">{d.sla?.breached || 0}</div><div className="rpt-kpi-label">SLA Breaches</div></div></div></div>

      <div className="rpt-section"><h2>Alert Distribution</h2><div className="rpt-bar-chart">{[{ label: 'Critical', count: sev.critical, color: '#ff4466' }, { label: 'High', count: sev.high, color: '#f97316' }, { label: 'Medium', count: sev.medium, color: '#ffb340' }, { label: 'Low', count: sev.low, color: '#5b9aff' }].map(s => (<div key={s.label} className="rpt-bar-row"><div className="rpt-bar-label">{s.label}</div><div className="rpt-bar-track"><div className="rpt-bar-fill" style={{ width: Math.max(2, (s.count / Math.max(d.alerts.length, 1)) * 100) + '%', background: s.color }} /></div><div className="rpt-bar-count">{s.count}</div></div>))}</div></div>

      {Object.keys(topSources).length > 0 && <div className="rpt-section"><h2>Alerts by Source</h2><div className="rpt-bar-chart">{Object.entries(topSources).sort((a, b) => b[1] - a[1]).map(([src, count]) => (<div key={src} className="rpt-bar-row"><div className="rpt-bar-label">{src}</div><div className="rpt-bar-track"><div className="rpt-bar-fill" style={{ width: Math.max(2, (count / d.alerts.length) * 100) + '%', background: '#5b9aff' }} /></div><div className="rpt-bar-count">{count}</div></div>))}</div></div>}

      {nrStats.totalProcessed > 0 && <div className="rpt-section"><h2>AI Noise Reduction</h2><div className="rpt-kpis"><div className="rpt-kpi"><div className="rpt-kpi-val" style={{ color: '#34e8a5' }}>{nrStats.autoClosed || 0}</div><div className="rpt-kpi-label">Auto-Closed</div></div><div className="rpt-kpi"><div className="rpt-kpi-val">{nrStats.escalated || 0}</div><div className="rpt-kpi-label">Escalated</div></div><div className="rpt-kpi"><div className="rpt-kpi-val" style={{ color: '#34e8a5' }}>{Math.round((nrStats.timeSavedMins || 0) / 60 * 10) / 10}h</div><div className="rpt-kpi-label">Time Saved</div></div><div className="rpt-kpi"><div className="rpt-kpi-val">{nrStats.totalProcessed > 0 ? Math.round(nrStats.autoClosed / nrStats.totalProcessed * 100) : 0}%</div><div className="rpt-kpi-label">FP Rate</div></div></div></div>}

      {d.alerts.length > 0 && <div className="rpt-section"><h2>Critical & High Alerts</h2><table className="rpt-tbl"><thead><tr><th>Alert</th><th>Source</th><th>Severity</th><th>Device</th><th>MITRE</th><th>Time</th></tr></thead><tbody>{d.alerts.filter((al: any) => al.severity === 'critical' || al.severity === 'high').slice(0, 20).map((al: any) => (<tr key={al.id}><td style={{ fontWeight: 600 }}>{al.title}</td><td>{al.source}</td><td><span style={{ color: al.severity === 'critical' ? '#ff4466' : '#f97316', fontWeight: 700 }}>{al.severity}</span></td><td style={{ fontSize: '.75rem' }}>{al.device || '—'}</td><td style={{ fontSize: '.72rem', color: '#8896b8' }}>{al.mitre || '—'}</td><td style={{ fontSize: '.72rem', color: '#8896b8' }}>{new Date(al.timestamp).toLocaleString()}</td></tr>))}</tbody></table></div>}
...
*{margin:0;padding:0;box-sizing:border-box}body{background:#f8fafd;color:#1a1a2e;font-family:'DM Sans',sans-serif}
.rpt{max-width:900px;margin:0 auto;padding:20px}
.rpt-header{display:flex;gap:8px;margin-bottom:20px}
.rpt-btn{padding:10px 20px;border:none;border-radius:10px;background:linear-gradient(135deg,#5b9aff,#8b6fff);color:#fff;font-size:.85rem;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif}
.rpt-btn-ghost{background:transparent;border:1px solid #d8e0ef;color:#4a5672}
...
.rpt-logo-icon{width:32px;height:32px;border-radius:8px;background:linear-gradient(135deg,#5b9aff,#8b6fff);display:flex;align-items:center;justify-content:center;font-size:.75rem;color:#fff;font-weight:900}
...
.rpt-org{font-size:.78rem;color:#8896b8;margin-top:2px}
...
.rpt-factor-name{font-size:.78rem;font-weight:600;min-width:130px;color:#4a5672}
...
.rpt-factor-val{font-size:.72rem;font-weight:700;font-family:'JetBrains Mono',monospace;min-width:36px;text-align:right;color:#4a5672}
...
.rpt-kpi-label{font-size:.62rem;font-weight:700;color:#8896b8;text-transform:uppercase;letter-spacing:.5px;margin-top:4px}
.rpt-bar-chart{display:flex;flex-direction:column;gap:8px}
.rpt-bar-row{display:flex;align-items:center;gap:10px}
.rpt-bar-label{font-size:.78rem;font-weight:600;min-width:80px;color:#4a5672}
...
.rpt-tbl th{text-align:left;padding:8px;font-size:.62rem;font-weight:700;color:#8896b8;text-transform:uppercase;letter-spacing:.5px;border-bottom:2px solid #e8ecf4}
.rpt-tbl td{padding:8px;border-bottom:1px solid #f0f3f9}
.rpt-footer{display:flex;justify-content:space-between;margin-top:40px;padding-top:16px;border-top:1px solid #eef2f9;font-size:.7rem;color:#8896b8}

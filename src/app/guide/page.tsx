...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Overview tab', body: 'Your SOC command centre. Shows: AI Shift Brief (session summary at the top), Estate Health (4 stat cards — devices, tool status, alerts ingested, vulns), Recent Alerts panel, Active Incidents panel, Top Threats strip, and the Security Posture gauge with a 7-day trend sparkline.\n\nClick the "N critical" count on the Alerts stat card to jump to the Alerts tab pre-filtered to Critical severity.' },
      ...
      { heading: 'Alerts tab', body: 'Every alert triaged by AI. Sort, filter by severity or source, or search by title/device/source. Click any alert row to expand full AI reasoning, evidence chain, confidence score, and actions taken.\n\nLive alerts get on-demand triage automatically when expanded. Demo alerts have pre-built enrichment.\n\nAlerts with a green ACK badge have been acknowledged. FP-marked alerts are struck-through and dimmed.' },
      ...
      { heading: 'Incidents tab', body: 'Correlated incidents with AI-generated attack narratives, MITRE ATT&CK kill chains, full timelines, and affected devices.\n\nThe "View source alerts →" button inside any expanded incident jumps to the Alerts tab with those exact source alerts expanded — so you can trace from case back to raw signal in one click.\n\nClose incidents when resolved — this logs the closure time and feeds back into posture scoring.' },
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Secureworks Taegis XDR', body: 'Fields: Client ID, Client Secret, Region (us1/eu1/us2). Auth: OAuth2 client credentials. Fetches open alerts with severity ≥ 0.6 via GraphQL.' },
      ...
      { heading: 'Tenable.io', body: 'Fields: Access Key, Secret Key. Auth: X-ApiKeys header. Fetches high/critical vulnerabilities from the last 7 days via workbenches API.' },

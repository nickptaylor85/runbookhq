...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Overview tab', body: 'Your SOC command centre. Shows:\n\n• 5 Hero Stats — Critical Alerts, Total Alerts, Open Cases, Critical Vulns, Posture Score. Each is clickable, jumping to the relevant tab.\n• AI Threat Brief — one-paragraph situational summary wired to live data.\n• 4-quadrant drill-down grid — Alerts by Severity, Estate Coverage, Top 5 Vulnerabilities, Active Cases + Tool Health.\n• 7-day Posture Sparkline.\n• Shift Metrics — full-width 4-column panel: Unacked Criticals, SLA Breaches, FPs Auto-Closed, Tools Live. At-a-glance shift health.' },
      ...
      { heading: 'Coverage tab', body: 'Shows which devices are covered by which tools, and which are unmonitored blind spots.\n\nHeader shows: coverage %, OS breakdown (Windows N, Linux N, macOS N), and total device count sourced from Tenable in live mode.\n\nEach gap device is colour-coded by how recently it was seen:\n🟢 Recent (today/yesterday)\n🟡 2–3 days ago\n🟠 4–7 days ago\n🔴 Stale — over 7 days (likely an agent failure)\n\nClick any tool in the coverage bar chart to filter gap devices to only those missing that specific tool.' },
      ...
      { heading: 'Intel tab', body: 'Live threat intelligence in three sections (top 3 each):\n\n1. [Industry] — Active Threats: threats specific to your selected sector.\n2. General Intelligence: broad threat landscape items — CISA KEV updates, ransomware activity, C2 feeds.\n3. Tenable — In The News: latest CVE disclosures from Tenable Research with CVSS, affected products, and patch guidance.\n\nSelect your sector from the Industry dropdown. Each item is expandable to view IOCs, MITRE mapping, and AI-generated hunt queries for Splunk, Sentinel, and Defender.' },
      { heading: 'Incidents tab', body: 'Correlated incidents with AI-generated attack narratives, MITRE ATT&CK kill chains, full timelines, and affected devices.\n\nExpanded view shows two sections:\n\n1. Attack Timeline — full chronological log of AI and analyst actions. AI entries (blue dot) show what the AI did; Analyst entries (green dot) show analyst decisions. Any automated command run is shown inline in a code block.\n\n2. AI Commands Executed — dedicated sub-panel extracting all AI-run commands from the timeline. Terminal-style view with timestamps, action labels, and copy buttons. This is the audit log of exactly what Watchtower AI did using your tools (isolations, account disables, IP blocks, queries run).\n\nSLA countdown is visible on every collapsed row without expanding.' },
      { heading: 'Tools tab', body: 'Connect and manage all integrations. Click "+ Connect" on any tool, enter credentials, and hit "Test Connection" to validate before saving.\n\nThe Sync Log is always visible at the top of the tab — a dark terminal-style panel showing every sync event in newest-first order. Each line shows: timestamp, tool name (colour-coded), record count, and duration in ms. Errors show in red. The log auto-scrolls to the latest entry on each sync.\n\nCredentials are encrypted at rest with AES-256-GCM. Secret fields are never returned to the browser after saving.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { heading: 'Secureworks Taegis XDR', body: 'Fields: Client ID, Client Secret, Region (us1/eu1/us2).\n\nAuth: OAuth2 client credentials. Auth endpoint is region-specific:\n• us1 → api.ctpx.secureworks.com\n• eu1/us2 → api.{region}.taegis.secureworks.com\n\nFetches detections via GraphQL alertsServiceSearch query using CQL:\nFROM alert WHERE severity >= 0.6 EARLIEST=-7d\n\nSeverity is a float 0.0–1.0 scale: >=0.9=Critical, >=0.7=High, >=0.4=Medium.\n\nNote: Taegis UI renamed "Alerts" to "Detections" and "Investigations" to "Cases" but the GraphQL API still uses the original terminology.' },
      ...
      { heading: 'Tenable.io', body: 'Fields: Access Key, Secret Key. Auth: X-ApiKeys header. Fetches High and Critical vulnerabilities from the last 90 days via /workbenches/vulnerabilities.\n\nVulnerabilities are grouped by plugin — each unique CVE/plugin appears once with a full list of all affected assets. The count ("7 devices affected") and the hostname list are both shown in the expanded view.\n\nPer-asset hostnames fetched via /workbenches/vulnerabilities/{id}/outputs in parallel. Note: the severity filter uses string values (High, Critical) per the Tenable API spec — not integers.' },
  ...
  },
  {
    ...
    items: [
      { heading: 'AI Query Log', body: 'Every AI call across the platform is logged and viewable by admins. Access via Admin Portal → ✦ AI Log tab.\n\nEach entry records: timestamp, user, tenant, query type (triage/copilot/intel/vuln), prompt preview, alert or vuln context, duration, and success/error status. Capped at 500 entries per tenant (rolling).\n\nUseful for:\n• Auditing how the AI is being used across your team\n• Diagnosing slow or failed AI responses\n• Understanding which alerts are triggering the most AI calls\n• Compliance evidence of AI-assisted triage decisions' },

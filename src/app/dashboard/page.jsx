          ...
          {activeTab==='coverage' && (
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                ...
                {!demoMode&&liveKnownDevices.length>0&&<span style={{fontSize:'0.6rem',color:'#00b3e3',background:'#00b3e310',padding:'2px 8px',borderRadius:4}}>✦ Live · {liveKnownDevices.length} devices</span>}
                <div style={{display:'flex',gap:5,flexWrap:'wrap',marginLeft:'auto'}}>
                  {Object.entries(osBreakdown).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([os,n])=>(
                    <span key={os} style={{fontSize:'0.58rem',padding:'2px 7px',borderRadius:4,background:'var(--wt-card2)',border:'1px solid var(--wt-border)',fontFamily:'JetBrains Mono,monospace',color:'var(--wt-secondary)'}}>{os} <strong>{n}</strong></span>
                  ))}
                </div>
              </div>

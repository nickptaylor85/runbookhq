  ...
  const liveKnownDevices = !demoMode && liveVulns.length > 0
    ? [...new Map(liveVulns.flatMap(v => {
        ...
        return assets.map(hostname => [hostname, {
          ...
          missing: connectedEdrNames.length > 0 ? [] : ['No EDR configured — connect CrowdStrike, Defender, or SentinelOne'],
          reason: connectedEdrNames.length > 0 ? 'Device scanned by Tenable — EDR coverage confirmed via connected tools' : 'Device found in Tenable scan — no EDR tool connected',
...
Generated by Watchtower`;
        ...
        <div className="wt-content wt-main" style={{flex:1,overflow:'auto',padding:'16px 18px',background:'var(--wt-bg)'}} key={activeTab}>
          ...
          {activeTab==='overview' && (
            <div style={{maxWidth:960,margin:'0 auto',width:'100%',display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}} className='wt-two-col'>
                ...
                {/* Noise reduction */}
                <div style={{background:'var(--wt-card)',border:'1px solid var(--wt-border)',borderRadius:12,padding:'14px'}}>
                  <div style={{fontSize:'0.62rem',fontWeight:800,color:'var(--wt-muted)',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>AI Noise Reduction</div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    {[
                      {label:'FPs Closed',value:fpAlerts.length,color:'#22d49a'},
                      {label:'TPs Escalated',value:tpAlerts.length,color:'#f0405e'},
                      {label:'Hours Saved',value:Math.round(fpAlerts.length*0.25),color:'#4f8fff'},
                      {label:'Auto Rate',value:`${Math.round((fpAlerts.length+tpAlerts.length)/Math.max(alerts.length,1)*100)}%`,color:'#f0a030'},
                    ].map(s=>(
                      <div key={s.label} style={{textAlign:'center',padding:'8px',background:'var(--wt-card2)',borderRadius:8}}>
                        <div style={{fontSize:'1.4rem',fontWeight:900,fontFamily:'JetBrains Mono,monospace',color:s.color}}>{s.value}</div>
                        <div style={{fontSize:'0.54rem',color:'var(--wt-dim)',marginTop:2}}>{s.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {activeTab==='intel' && (
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div>
                ...
                {allIntel.filter(i=>i.industrySpecific).slice(0,3).map(item=>{
              ...
              {/* General intel — top 3 */}
              <div>
                <div style={{fontSize:'0.62rem',fontWeight:700,color:'var(--wt-muted)',textTransform:'uppercase',letterSpacing:'1px',marginBottom:8,display:'flex',alignItems:'center',gap:8}}>General Intelligence<span style={{fontSize:'0.54rem',fontWeight:600,color:'var(--wt-dim)',fontFamily:'JetBrains Mono,monospace'}}>top 3</span></div>
                {allIntel.filter(i=>!i.industrySpecific&&!i.tenableNews).slice(0,3).map(item=>{
                  const isExpanded = expandedIntel.has(item.id);
                  return (
          ...
          {/* ═══════════════════════════════ MSSP PORTFOLIO ══════════════════════════ */}
          {activeTab==='mssp' && <MSSPPortfolio currentTenant={currentTenant} setCurrentTenant={setCurrentTenantWithRef} DEMO_TENANTS={DEMO_TENANTS} isAdmin={isAdmin} setActiveTab={setActiveTab} setAdminBannerInput={setAdminBannerInput} />}

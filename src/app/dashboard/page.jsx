...
const DASHBOARD_CSS = `*{margin:0;padding:0;box-sizing:border-box}
        ...
        .wt-root {
          ...
          --cp-blue:     #2090ff;
          --cp-red:      #ff3355;
          --glow-cyan:   0 0 10px #00d4f040,0 0 2px #00d4f0;
          --glow-red:    0 0 10px #ff335540,0 0 2px #ff3355;
          --glow-green:  0 0 10px #00e87a40,0 0 2px #00e87a;
          --glow-amber:  0 0 10px #ffb30040,0 0 2px #ffb300;
          --glow-purple: 0 0 10px #b060ff40,0 0 2px #b060ff;
        }
        /* ── CYBERPUNK LIGHT ── */
        .wt-root.light {
          --wt-bg:       #f0f4ff;
        ...
        .wt-root:not(.light) .alert-card{border-color:rgba(0,212,240,0.15)}
        .wt-root:not(.light) .alert-card:hover{border-color:rgba(0,212,240,0.35);box-shadow:0 0 10px rgba(0,212,240,0.07)}
...
@media(max-width:640px){
        ...
        /* ══════════════════════════════════════════════
           GLASSMORPHISM · AMBIENT LIGHT · PREMIUM BORDERS
           ══════════════════════════════════════════════ */

        /* Deep space background + dot grid */
        .wt-root::before {
          content: '';
          position: fixed;
          inset: 0;
          z-index: 0;
          pointer-events: none;
          background: radial-gradient(ellipse 90% 70% at 8% 12%, rgba(0,80,255,0.12) 0%, transparent 55%), radial-gradient(ellipse 70% 60% at 94% 88%, rgba(130,0,255,0.10) 0%, transparent 55%), radial-gradient(ellipse 50% 40% at 50% 48%, rgba(0,200,240,0.06) 0%, transparent 65%), radial-gradient(ellipse 55% 45% at 18% 82%, rgba(0,40,180,0.08) 0%, transparent 55%), radial-gradient(ellipse 40% 35% at 80% 20%, rgba(80,0,200,0.07) 0%, transparent 50%), radial-gradient(rgba(0,180,240,0.045) 1px, transparent 1px), #060c18;
        ...
        @keyframes orb-drift-c { 0%,100%{ transform:translate(0,0) scale(1); } 50%{ transform:translate(26px,38px) scale(1.07); } }
        .wt-orb-a,.wt-orb-b,.wt-orb-c{ position:fixed!important;border-radius:50%;pointer-events:none;z-index:0;mix-blend-mode:screen; }
        ...
        .wt-orb-c{ width:460px;height:460px;top:38%;left:52%; background:radial-gradient(circle,rgba(0,210,255,0.08) 0%,transparent 65%); animation:orb-drift-c 32s ease-in-out infinite; }
        ...
        .wt-topbar {
          ...
          box-shadow: 0 1px 0 rgba(255,255,255,0.04), 0 8px 36px rgba(0,0,0,0.55), inset 0 0 60px rgba(0,80,255,0.025) !important;
        }

        /* Tab active state */
        .tab-btn.active { background: rgba(0,220,255,0.09) !important; color: #00e5ff !important; box-shadow: inset 0 0 0 1px rgba(0,220,255,0.32), 0 0 18px rgba(0,220,255,0.14) !important; }

        /* Alert cards - frosted glass with shimmer top */
        .alert-card { background: rgba(8,16,36,0.60) !important; backdrop-filter: blur(18px) saturate(1.5) !important; -webkit-backdrop-filter: blur(18px) saturate(1.5) !important; border: 1px solid rgba(0,180,240,0.20) !important; box-shadow: 0 2px 16px rgba(0,0,0,0.45), 0 1px 0 rgba(255,255,255,0.06) inset, 0 -1px 0 rgba(0,0,0,0.25) inset !important; transition: border-color .2s, box-shadow .2s, transform .15s !important; position: relative !important; overflow: hidden !important; }
        .alert-card::after { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent 0%, rgba(0,212,240,0.55) 25%, rgba(180,130,255,0.45) 65%, transparent 100%); z-index: 2; pointer-events: none; }
        .wt-root:not(.light) .alert-card { border-color: rgba(0,180,240,0.20) !important; }
        .alert-card:hover { border-color: rgba(0,212,240,0.45) !important; box-shadow: 0 8px 36px rgba(0,0,0,0.55), 0 0 28px rgba(0,212,240,0.10), 0 1px 0 rgba(255,255,255,0.08) inset !important; transform: translateY(-1px) !important; }
        .wt-root:not(.light) .alert-card:hover { border-color: rgba(0,212,240,0.45) !important; }

        /* All wt-card surfaces get glass border + shimmer */
        .wt-root:not(.light) [style*="background:var(--wt-card)"], .wt-root:not(.light) [style*="background: var(--wt-card)"] { backdrop-filter: blur(14px) saturate(1.3) !important; -webkit-backdrop-filter: blur(14px) saturate(1.3) !important; box-shadow: 0 0 0 1px rgba(0,180,240,0.18), 0 4px 22px rgba(0,0,0,0.42), 0 1px 0 rgba(255,255,255,0.055) inset !important; }

        /* wt-card2 surfaces */
        .wt-root:not(.light) [style*="background:var(--wt-card2)"], .wt-root:not(.light) [style*="background: var(--wt-card2)"] { backdrop-filter: blur(10px) !important; -webkit-backdrop-filter: blur(10px) !important; box-shadow: 0 0 0 1px rgba(0,180,240,0.11), inset 0 1px 0 rgba(255,255,255,0.03) !important; }

        /* Content area transparent */
        .wt-content, .wt-main { background: transparent !important; }

        /* Co-Pilot panel */
        .wt-copilot { background: rgba(5,10,25,0.90) !important; backdrop-filter: blur(32px) saturate(1.6) !important; -webkit-backdrop-filter: blur(32px) saturate(1.6) !important; border: 1px solid rgba(0,200,255,0.20) !important; box-shadow: 0 20px 72px rgba(0,0,0,0.65), 0 0 0 1px rgba(0,200,255,0.08), inset 0 1px 0 rgba(255,255,255,0.05) !important; }

        /* Bottom nav */
        .wt-bottom-nav { background: rgba(4,8,20,0.90) !important; backdrop-filter: blur(24px) !important; -webkit-backdrop-filter: blur(24px) !important; border-top: 1px solid rgba(0,180,240,0.16) !important; box-shadow: 0 -4px 28px rgba(0,0,0,0.5) !important; }

        /* Grain overlay */
        .wt-root::after { content: ''; position: fixed; inset: 0; z-index: 9999; pointer-events: none; opacity: 0.022; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E"); background-size: 180px 180px; }

        /* Scrollbars */
        .wt-root ::-webkit-scrollbar { width: 5px; height: 5px; }
        .wt-root ::-webkit-scrollbar-track { background: transparent; }
        .wt-root ::-webkit-scrollbar-thumb { background: rgba(0,180,240,0.22); border-radius: 3px; }
        .wt-root ::-webkit-scrollbar-thumb:hover { background: rgba(0,180,240,0.40); }

        /* Stat glow helpers */
        .wt-root:not(.light) .wt-stat-val-cyan  { text-shadow: 0 0 24px rgba(0,212,240,0.6); }
        .wt-root:not(.light) .wt-stat-val-red    { text-shadow: 0 0 24px rgba(255,50,85,0.6); }
        .wt-root:not(.light) .wt-stat-val-green  { text-shadow: 0 0 24px rgba(0,232,122,0.6); }
        .wt-root:not(.light) .wt-stat-val-amber  { text-shadow: 0 0 24px rgba(255,179,0,0.6); }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={onClose}>
      <div style={{background:'var(--wt-card2)',border:'1px solid var(--wt-border2)',borderRadius:16,maxWidth:700,width:'100%',maxHeight:'85vh',overflow:'auto',position:'relative'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',alignItems:'center',padding:'16px 20px',borderBottom:'1px solid rgba(0,212,240,0.15)',position:'sticky',top:0,background:'var(--wt-card2)',zIndex:10}}>
...
function StatCard({val,label,sub,color,onClick}) {
  return (
    <div onClick={onClick} style={{padding:'14px 12px',background:'var(--wt-card)',border:'1px solid var(--wt-border)',borderRadius:10,textAlign:'center',cursor:onClick?'pointer':'default',transition:'border-color .15s'}}
...
export default function DashboardPage() {
  ...
  const alerts = rawAlerts.map(a => alertOverrides[a.id] ? {...a, ...alertOverrides[a.id]} : a).filter(a => !a.deleted);
  ...
  return (
    <div className={`wt-root${theme === 'light' ? ' light' : ''}${digitalFont ? ' wt-digital' : ''}`} style={{display:'flex',flexDirection:'column',minHeight:'100vh',maxWidth:'100vw',overflowX:'hidden',background:'#060c18',color:'var(--wt-text)',fontFamily:"'Rajdhani','JetBrains Mono',monospace"}}>
      <style dangerouslySetInnerHTML={{__html:DASHBOARD_CSS}} />
      <div className="wt-orb-a" aria-hidden="true" />
      <div className="wt-orb-b" aria-hidden="true" />
      <div className="wt-orb-c" aria-hidden="true" />
      ...
      <div style={{display:'flex',flexDirection:'column',overflow:'hidden',flex:1,maxWidth:'100vw'}}>
        ...
        <div className="wt-topbar" style={{display:'flex',alignItems:'center',padding:'0 16px',borderBottom:'1px solid rgba(0,212,240,0.15)',background:'var(--wt-sidebar)',flexShrink:0,gap:0,height:48}}>
          {/* Logo + wordmark */}
          <div style={{display:'flex',alignItems:'center',gap:8,paddingRight:16,borderRight:'1px solid var(--wt-border)',marginRight:8,flexShrink:0,height:'100%'}}>
          ...
          <div className="wt-topbar-controls-full" style={{marginLeft:'auto',alignItems:'center',gap:8,height:'100%'}}>
            {/* Sync status */}
            <div style={{display:'flex',alignItems:'center',gap:5,fontSize:'0.84rem',color:'var(--wt-muted)',borderRight:'1px solid var(--wt-border)',paddingRight:10,height:'100%'}}>
            ...
            {/* Automation */}
            {(canUse('team')||isAdmin) ? (
              <div style={{display:'flex',alignItems:'center',gap:3,padding:'3px 8px',borderRadius:7,background:'var(--wt-card2)',border:'1px solid var(--wt-border)'}}>
                            ...
                            <div style={{display:'flex',flexDirection:'column',gap:4}}>
                              {(inc.timeline||[]).filter(e=>e.actor==='AI'&&e.cmd).map((e,ci)=>(
                                <div key={ci} style={{display:'flex',gap:8,alignItems:'flex-start'}}>
                                  ...
                                  <button onClick={()=>navigator.clipboard.writeText(e.cmd)} style={{padding:'2px 6px',borderRadius:3,border:'1px solid var(--wt-border)',background:'transparent',color:'var(--wt-dim)',fontSize:'0.84rem',cursor:'pointer',fontFamily:"'Rajdhani','JetBrains Mono',monospace",flexShrink:0,marginTop:3}}>Copy</button>
                                </div>
                              ))}
                            </div>
                          </div>
                ...
                <div style={{marginLeft:'auto',display:'flex',gap:6}}>
                  ...
                  <button onClick={async()=>{
                    try{
                      ...
                      const d=await r.json();
                      if(d.html){
                        const sr=await fetch('/api/report-share',{method:'POST',headers:{'Content-Type':'application/json','x-tenant-id':tenantRef.current},body:JSON.stringify({html:d.html,title:'Security Report'})});
      ...
      {modal?.type==='gaps' && (
        <Modal title={`Coverage Gaps — ${gapDevices.length} Devices`} onClose={()=>setModal(null)}>
          ...
          {gapDevices.map(dev=>(
            <div key={dev.hostname} style={{padding:'12px 0',borderBottom:'1px solid rgba(0,212,240,0.15)',cursor:'pointer'}} onClick={()=>{setModal(null);setDeployAgentDevice(dev);}}>
      ...
      {modal?.type==='tools' && (
        <Modal title='Tool Status & Sync' onClose={()=>setModal(null)}>
          ...
          {tools.filter(t=>t.active||connectedTools[t.id]).map(tool=>{
            ...
            return (
              <div key={tool.id} style={{padding:'10px 0',borderBottom:'1px solid rgba(0,212,240,0.15)'}}>
      ...
      {modal?.type==='alerts-ingested' && (
        <Modal title={`Alert Ingestion — What AI Did`} onClose={()=>setModal(null)}>
          ...
          {alerts.map(a=>(
            <div key={a.id} style={{padding:'10px 0',borderBottom:'1px solid rgba(0,212,240,0.15)'}}>

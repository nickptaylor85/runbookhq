        ...
        <div className="wt-content wt-main" style={{flex:1,overflow:'auto',padding:'16px 18px',background:'var(--wt-bg)'}} key={activeTab}>
          ...
          {activeTab==='overview' && (
            <div style={{maxWidth:960,margin:'0 auto',width:'100%',display:'flex',flexDirection:'column',gap:14}}>
              ...
              {/* ── AI THREAT BRIEF ──────────────────────────────────────────────── */}
              <div style={{background:'linear-gradient(135deg,rgba(79,143,255,0.06),rgba(34,201,146,0.04))',border:'1px solid #4f8fff25',borderRadius:12,padding:'14px 18px'}}>
                ...
                <div style={{fontSize:'0.76rem',color:'var(--wt-secondary)',lineHeight:1.7}}>
                  {critAlerts.length > 0
                    ? <span><strong style={{color:'#f0405e'}}>{critAlerts.length} critical alert{critAlerts.length!==1?'s':''}</strong> require immediate attention{critAlerts[0]? ` — ${critAlerts[0].title}`:''}.{' '}Estate coverage {coveredPct}%{gapDevices.length>0?` — ${gapDevices.length} unmonitored device${gapDevices.length!==1?'s':''}`:''}. {kevVulns.length>0?`${kevVulns.length} CISA KEV vulns — patch within 72h. `:''}Posture {posture}/100.</span>
                    : <span>{demoMode?'Demo mode active':'Live'} — {totalAlerts} alert{totalAlerts!==1?'s':''} ingested, no critical threats. Posture {posture}/100. {kevVulns.length>0?`${kevVulns.length} KEV items pending. `:''}Coverage {coveredPct}%.</span>
                  }
                </div>
              </div>

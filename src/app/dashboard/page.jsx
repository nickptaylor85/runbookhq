    ...
    <div className={`wt-root${theme === 'light' ? ' light' : ''}`} style={{display:'flex',minHeight:'100vh',background:'var(--wt-bg)',color:'var(--wt-text)',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{display:'flex',alignItems:'center',padding:'8px 18px',borderBottom:'1px solid #141820',gap:12,background:'var(--wt-sidebar)',flexShrink:0,flexWrap:'wrap'}}>
          ...
          <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:5,fontSize:'0.7rem',color:'var(--wt-muted)'}}>
              ...
              {!demoMode && syncStatus==='ok' && <span style={{display:'inline-flex',alignItems:'center',gap:5}}><span style={{width:6,height:6,borderRadius:'50%',background:'#22c992',boxShadow:'0 0 6px #22c992',display:'block'}} />{Object.keys(connectedTools).length} connected</span>}
        ...
        <div style={{flex:1,overflow:'auto',padding:'16px 18px',background:'var(--wt-bg)'}}>
          ...
          {/* ═══════════════════════════════ OVERVIEW ═══════════════════════════════ */}
            <OverviewTab setSelectedIncident={setSelectedIncident} incidentStatuses={incidentStatuses} setModal={setModal} activeTab={activeTab} setActiveTab={setActiveTab} alerts={alerts} vulns={vulns} incidents={incidents} tools={tools} activeTools={activeTools} critAlerts={critAlerts} tpAlerts={tpAlerts} fpAlerts={fpAlerts} critVulns={critVulns} kevVulns={kevVulns} coveredPct={coveredPct} totalDevices={totalDevices} gapDevices={gapDevices} />
            {activeTab==='alerts' && (
              <AlertsTab
                alerts={alerts}
                demoMode={demoMode}
                ...
                alertSevFilter={alertSevFilter}
                setAlertSevFilter={setAlertSevFilter}
                alertSrcFilter={alertSrcFilter}
                setAlertSrcFilter={setAlertSrcFilter}
                alertSort={alertSort}
                setAlertSort={setAlertSort}
                alertPage={alertPage}
                setAlertPage={setAlertPage}
                alertTotalPages={alertTotalPages}
                alertPageClamped={alertPageClamped}
                ...
                setAlertOverrides={setAlertOverrides}
                aiTriageCache={aiTriageCache}
                createdIncidents={createdIncidents}
                setCreatedIncidents={setCreatedIncidents}
          ...
          {/* ═══════════════════════════════ COVERAGE ═══════════════════════════════ */}
            <CoverageTab setDeployAgentDevice={setDeployAgentDevice} gapToolFilter={gapToolFilter} setGapToolFilter={setGapToolFilter} activeTab={activeTab} activeTools={activeTools} coveredPct={coveredPct} totalDevices={totalDevices} gapDevices={gapDevices} />
            <VulnsTab selectedVuln={selectedVuln} setSelectedVuln={setSelectedVuln} vulnAiTexts={vulnAiTexts} setVulnAiTexts={setVulnAiTexts} vulnAiLoading={vulnAiLoading} activeTab={activeTab} vulns={vulns} kevVulns={kevVulns} getVulnAiHelp={getVulnAiHelp} />
            <IntelTab industry={industry} userTier={userTier} intelLoading={intelLoading} expandedIntel={expandedIntel} iocQueryLoading={iocQueryLoading} setIocQueryLoading={setIocQueryLoading} iocQueries={iocQueries} setIocQueries={setIocQueries} activeTab={activeTab} allIntel={allIntel} toggleIntel={toggleIntel} fetchIntelForIndustry={fetchIntelForIndustry} />
            <IncidentsTab userTier={userTier} selectedIncident={selectedIncident} setSelectedIncident={setSelectedIncident} incidentStatuses={incidentStatuses} setIncidentStatuses={setIncidentStatuses} deletedIncidents={deletedIncidents} incidentNotes={incidentNotes} setIncidentNotes={setIncidentNotes} addingNoteTo={addingNoteTo} setAddingNoteTo={setAddingNoteTo} activeTab={activeTab} noteInput={noteInput} setNoteInput={setNoteInput} alerts={alerts} incidents={incidents} closeIncident={closeIncident} deleteIncident={deleteIncident} />
          {activeTab==='tools' && (

          ...
          <div className="wt-tabbar" style={{display:'flex',gap:2,flexWrap:'wrap'}}>
            {TABS.filter(t=>{
              ...
              <button key={t} className={`tab-btn${activeTab===t?' active':''}`} onClick={()=>setActiveTab(t)}>
                {t==='mssp'?'Portfolio':t==='compliance'?'🛡 Compliance':t.charAt(0).toUpperCase()+t.slice(1)}{t==='alerts'&&critAlerts.length>0&&<span style={{marginLeft:5,fontSize:'0.48rem',fontWeight:800,padding:'1px 5px',borderRadius:3,background:'#f0405e',color:'#fff'}}>{critAlerts.length}</span>}
...
Generated by Watchtower`;
        ...
        <div className="wt-content wt-main" style={{flex:1,overflow:'auto',padding:'16px 18px',background:'var(--wt-bg)'}} key={activeTab}>
          ...
          {activeTab==='overview' && (
            <div style={{maxWidth:960,margin:'0 auto',width:'100%',display:'flex',flexDirection:'column',gap:16}}>
              ...
              {/* First-login welcome card — live mode, no tools connected */}
              {!demoMode && Object.keys(connectedTools).length===0 && (
                <div style={{padding:'18px 20px',background:'linear-gradient(135deg,rgba(79,143,255,0.07),rgba(139,111,255,0.05))',border:'1px solid #4f8fff30',borderRadius:14}}>
                  <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
                    <span style={{fontSize:'1.4rem'}}>🚀</span>
                    <div>
                      <div style={{fontWeight:800,fontSize:'0.92rem',marginBottom:2}}>Welcome to Watchtower</div>
                      <div style={{fontSize:'0.72rem',color:'var(--wt-muted)'}}>3 steps to a live SOC dashboard</div>
                    </div>
                  </div>
                  <div style={{display:'flex',flexDirection:'column',gap:6}}>
                    {[{n:1,label:'Add your Anthropic API key',desc:'Required for AI triage',tab:'tools'},{n:2,label:'Connect your first security tool',desc:'CrowdStrike, Splunk, Sentinel and 15 more',tab:'tools'},{n:3,label:'Switch to Live mode',desc:'Alerts sync every 60 seconds',tab:null}].map(s=>(
                      <div key={s.n} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:'var(--wt-card)',borderRadius:8,border:'1px solid var(--wt-border)'}}>
                        <div style={{width:22,height:22,borderRadius:'50%',background:'#4f8fff18',border:'2px solid #4f8fff40',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.58rem',fontWeight:900,color:'#4f8fff',flexShrink:0}}>{s.n}</div>
                        <div style={{flex:1}}><div style={{fontSize:'0.76rem',fontWeight:700}}>{s.label}</div><div style={{fontSize:'0.62rem',color:'var(--wt-muted)'}}>{s.desc}</div></div>
                        {s.tab&&<button onClick={()=>setActiveTab(s.tab)} style={{padding:'3px 10px',borderRadius:5,border:'1px solid #4f8fff30',background:'#4f8fff0f',color:'#4f8fff',fontSize:'0.65rem',fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',flexShrink:0}}>Go →</button>}
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:12,padding:16,background:'var(--wt-card)',border:'1px solid #1d2535',borderRadius:12}}>
                <div style={{position:'relative',width:64,height:64,flexShrink:0}}>
                  ...
                  <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,45%)',fontSize:'0.6rem',fontWeight:800,color:postureColor}}>{posture>=90?'A':posture>=80?'B':posture>=65?'C':posture>=50?'D':'F'}{posture>=90||posture>=80||posture>=65?'+':''}</div>
                </div>
                <div style={{flex:1}}>
      ...
      <nav className="wt-bottom-nav">
        {[{t:'overview',i:'📊',l:'Overview'},{t:'alerts',i:'🔔',l:'Alerts'},{t:'coverage',i:'🛡',l:'Coverage'},{t:'vulns',i:'🔍',l:'Vulns'},{t:'intel',i:'🌐',l:'Intel'},{t:'incidents',i:'📋',l:'Cases'},{t:'tools',i:'🔌',l:'Tools'},...(canUse('business')?[{t:'compliance',i:'🛡',l:'Comply'}]:[])].map(({t,i,l})=>(

...
Generated by Watchtower`;
        ...
        <div className="wt-content wt-main" style={{flex:1,overflow:'auto',padding:'16px 18px',background:'var(--wt-bg)'}} key={activeTab}>
          ...
          {!demoMode && Object.keys(connectedTools).length > 0 && canTeam && (
            <div id="wt-ai-key-nudge" style={{display:'none'}} ref={el=>{
              if(el){
                fetch('/api/settings/user',{headers:{'x-tenant-id':tenantRef.current}}).then(r=>r.json()).then(d=>{
                  if(!d.settings?.anthropic_api_key && el) el.style.display='block';
                }).catch(()=>{});
              }
            }}>
              ...
              <div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:6}}>
                  {activeTools.map(tool=>{
                    // In live mode: derive coverage from actual sync data
                    // Tenable/Qualys/Nessus — use liveVulns asset count as the known estate
                    // EDR/SIEM tools — compare known devices vs gap devices
                    ...
                    // Live: vuln tools cover 100% of what they scan; EDR tools: compare devices with gaps
            ...
            <div style={{display:'flex',flexDirection:'column',gap:0}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:14}}>
                {(()=>{
                  ...
                  const sorted = [...vulns].sort((a,b)=>sevScore(b)-sevScore(a));
                  const groups = {};
                  sorted.forEach(v => {
          ...
          {activeTab==='intel' && (
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {/* Industry-specific intel first — top 3 */}
              <div>
                <div style={{fontSize:'0.62rem',fontWeight:700,color:'#f0405e',textTransform:'uppercase',letterSpacing:'1px',marginBottom:8,display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span style={{fontSize:'0.54rem',fontWeight:600,color:'var(--wt-dim)',fontFamily:'JetBrains Mono,monospace'}}>top 3</span>
                </div>
                {industryIntel.filter(i=>i.industrySpecific).slice(0,3).map(item=>{
              ...
              <div>
                ...
                {tenableNewsItems.slice(0,3).map(item=>(
                  <div key={item.id} style={{background:'var(--wt-card)',border:'1px solid #00b3e318',borderRadius:10,marginBottom:6,padding:'12px 14px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:4}}>
                      <SevBadge sev={item.severity} />
                      <span style={{fontSize:'0.78rem',fontWeight:700,flex:1}}>{item.title}</span>
                    </div>
                    ...
                    <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                      ...
                      {item.mitre&&<span style={{fontSize:'0.52rem',color:'#7c6aff',fontFamily:'JetBrains Mono,monospace'}}>{item.mitre}</span>}
                      <a href={item.url} target='_blank' rel='noopener noreferrer' onClick={e=>e.stopPropagation()} style={{fontSize:'0.52rem',color:'#00b3e3',textDecoration:'none',padding:'1px 6px',border:'1px solid #00b3e320',borderRadius:3,background:'#00b3e30a'}}>Read article →</a>
                    </div>
                  </div>
                ))}
              </div>

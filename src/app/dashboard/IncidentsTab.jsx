      ...
      <GateWall feature='Incident Management' requiredTier='team' userTier={userTier} isAdmin={isAdmin}>
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}>
            ...
            <span style={{fontSize:'0.62rem',color:'#f0405e',background:'#f0405e25',padding:'2px 8px',borderRadius:4}}>{incidents.filter(i=>i.status==='Active').length} Active</span>
          </div>
                ...
                <div style={{padding:'12px 16px',cursor:'pointer',display:'flex',alignItems:'flex-start',gap:12}} onClick={()=>setSelectedIncident(isSel?null:inc)}>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                      ...
                      {investigation && <span style={{fontSize:'0.5rem',fontWeight:700,padding:'1px 6px',borderRadius:3,background:'#8b6fff28',color:'#8b6fff',border:'1px solid #8b6fff25'}}>✦ Investigated</span>}
                    </div>
                ...
                {isSel && (
                  <div style={{borderTop:'1px solid #1d2535',padding:'14px 16px'}}>
                    <GateWall feature='AI Attack Narrative' requiredTier='team' userTier={userTier} isAdmin={isAdmin}>
                      <div style={{fontSize:'0.74rem',color:'var(--wt-secondary)',lineHeight:1.65,padding:'10px',background:'linear-gradient(135deg,rgba(79,143,255,0.12),rgba(34,201,146,0.12))',border:'1px solid #4f8fff30',borderRadius:8,marginBottom:12}}>
                    ...
                    <div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>
                      ...
                      <button onClick={()=>setIncidentStatuses(prev=>({...prev,[inc.id]:'Escalated'}))} style={{padding:'5px 12px',borderRadius:6,border:'1px solid #f0a03030',background:'#f0a03020',color:'#f0a030',fontSize:'0.68rem',fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>⬆ Escalate</button>
                      <button onClick={()=>closeIncident(inc.id)} style={{padding:'5px 12px',borderRadius:6,border:'1px solid #22d49a30',background:'#22d49a20',color:'#22d49a',fontSize:'0.68rem',fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>✓ Close</button>
                      <button onClick={()=>deleteIncident(inc.id)} style={{padding:'5px 12px',borderRadius:6,border:'1px solid #f0405e25',background:'#f0405e20',color:'#f0405e',fontSize:'0.68rem',fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>🗑 Delete</button>
                    </div>
                    {showingInvest && (
                      <div style={{marginTop:14,background:'linear-gradient(135deg,rgba(139,111,255,0.13),rgba(79,143,255,0.10))',border:'1px solid #8b6fff25',borderRadius:12,overflow:'hidden'}}>
                        ...
                        {investigation && (
                          <div style={{padding:'14px',display:'flex',flexDirection:'column',gap:14}}>
                            ...
                            {investigation.attackTimeline?.length>0 && <div><div style={{fontSize:'0.56rem',fontWeight:700,color:'#4f8fff',textTransform:'uppercase',marginBottom:6}}>Reconstructed Attack Timeline</div>{investigation.attackTimeline.map((ev,i)=>(<div key={i} style={{display:'flex',gap:8,padding:'5px 0',borderBottom:'1px solid var(--wt-border)',alignItems:'flex-start'}}><span style={{fontSize:'0.6rem',fontFamily:'JetBrains Mono,monospace',color:'#4f8fff',flexShrink:0,minWidth:36}}>{ev.time}</span><div style={{flex:1}}><div style={{fontSize:'0.66rem',fontWeight:600,color:'var(--wt-text)',marginBottom:1}}>{ev.event}</div><div style={{fontSize:'0.6rem',color:'var(--wt-dim)'}}>{ev.significance}</div></div><span style={{fontSize:'0.52rem',color:'#4f8fff',fontWeight:700,flexShrink:0,padding:'1px 5px',background:'#4f8fff25',borderRadius:3}}>{ev.source}</span></div>))}</div>}

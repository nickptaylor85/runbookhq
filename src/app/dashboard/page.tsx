...
function LockedFeature({featureId,plan,children}:{featureId:string;plan:string;children:React.ReactNode}){
  ...
  const addonMap:Record<string,string>={ai_triage:'ai',ai_copilot:'ai',noise_reduction:'ai',intel:'intel',pdf_reports:'reports',api_access:'api',exec_summary:'ai',white_label:'branding'};
  ...
  const addons:string[]=(typeof window!=='undefined'?(window as any).__wt_addons:[])||[];
  const neededAddon=addonMap[featureId];
  if(userPlanIdx>=minPlanIdx&&(!neededAddon||addons.includes(neededAddon)))return <>{children}</>;
...
function IntelTab({alerts,onAskAI,onDeviceDrill}:{alerts:Al[];onAskAI?:(a:Al)=>void;onDeviceDrill?:(h:string)=>void}){
  return <><AIInsights tab="intel"/><ThreatIntelFeed/><MitreHeatmap alerts={alerts}/><TrendCharts/></>;
}
...
function DashboardInner(){
  ...
  const[critPopup,setCritPopup]=useState<any>(null);const critDismissedRef=React.useRef<Set<string>>(new Set());
  ...
  useEffect(()=>{
    ...
    if(critCount>prevCritCount&&prevCritCount>=0&&alerts.length>0){
      ...
      const unDismissed=newCrits.filter((a:any)=>!critDismissedRef.current.has(a.id||a.title));if(unDismissed.length>0)setCritPopup(unDismissed[0]);
      try{
...
function ShiftHandover({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  return <div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:560}}><div className="modal-hd"><div><h3 style={{fontSize:'.95rem'}}>🔄 Shift Handover</h3><p className="muted" style={{fontSize:'.65rem'}}>Last 8 hours summary</p></div><button className="modal-close" onClick={onClose}>✕</button></div><div className="modal-body"</>;
...
function Ov({m,cov,alerts,zsc,sparks,enabledTools,onAskAI,onRefresh,setTlDetail,onQuickAction,slaData}:any){
  return <><AIInsights tab="overview"/><div className="war-room"><div className="wr-left"><div style={{display:'flex',gap:10,alignItems:'flex-start',marginBottom:10,flexWrap:'wrap'}}><PostureGauge/><div style={{flex:1,minWidth:200}}><KpiSection m={m} cov={cov} zsc={zsc} sparks={sparks} enabledTools={enabledTools} alerts={alerts}/></div></div><SLABar sla={slaData}/><QuickActions onAction={onQuickAction}/><LockedFeature featureId="noise_reduction" plan={(typeof window!=='undefined'?(window as any).__wt_plan:null)||'community'}><NoiseReduction/></LockedFeature></div><div className="wr-right"><AlertStream alerts={alerts} onAlert={onAskAI}/></div></div><OvCharts m={m} sparks={sparks} enabledTools={enabledTools}/></>;
}
...
function AIInsights({tab}:{tab:string}){
  ...
  return <div className="ai-insights"><div className="ai-insights-hd"><span className="ai-insights-dot"/>AI Insights</div>{insights.map(i=>(<div key={i.id} className="ai-insight" style={{borderLeftColor:prioCol[i.priority]||'var(--accent)'}}><div className="ai-insight-top"><span className="ai-insight-icon">{i.icon}</span><span className="ai-insight-title">{i.title}</span></div><div className="ai-insight-detail">{i.detail}</div>{i.action&&<button className="tc-btn tc-btn-primary" onClick={()=>{if(i.actionTab){const e=new CustomEvent('wt-tab',{detail:i.actionTab});window.dispatchEvent(e)}else{alert('Action: '+i.action+'\n\n'+i.detail)}}} style={{fontSize:'.62rem',padding:'4px 10px',marginTop:6}}>{i.action}</button>}</div>))}</div>;
}
...
.shell-content{flex:1;display:flex;flex-direction:column;min-height:100vh;overflow-x:hidden}
.topbar{display:flex;align-items:center;gap:10px;padding:0 16px;height:44px;background:var(--bg0)f0;border-bottom:1px solid #ffffff06;position:sticky;top:0;z-index:100;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}
...
.crit-popup{margin:8px;max-width:calc(100vw - 16px)}
...
.crit-popup{margin:10px;max-width:calc(100vw - 20px)}
}
max-width:380px){
...
.shell-content{margin-left:52px;transition:margin-left .2s ease}
...
.side-logo .logo-icon{width:28px;height:28px;min-width:28px;border-radius:8px;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-size:.7rem;color:#fff;font-weight:900;box-shadow:0 2px 8px rgba(91,154,255,.2)}
.side-label{font-size:.78rem;font-weight:700;color:var(--t1);opacity:0;transition:opacity .15s;white-space:nowrap;overflow:hidden}
.sidebar.open .side-label{opacity:1}
.side-nav{flex:1;padding:8px 6px;display:flex;flex-direction:column;gap:2px;overflow-y:auto}
...
.side-icon{font-size:1rem;min-width:20px;text-align:center;flex-shrink:0}
.side-bottom{padding:8px 6px;border-top:1px solid var(--brd);display:flex;flex-direction:column;gap:2px}
.side-toggle{width:32px;height:32px;border:none;background:none;color:var(--t2);cursor:pointer;font-size:1rem;display:none;border-radius:6px}
.side-toggle:hover{background:var(--bg3)}
.tab-badge{font-size:.5rem;background:var(--red);color:#fff;padding:0 4px;border-radius:8px;margin-left:4px;font-weight:700}
...
.side-toggle{display:flex!important;align-items:center;justify-content:center}
...
.filter-row{flex-wrap:wrap;gap:3px}
.pills{overflow-x:auto;flex-wrap:nowrap;gap:2px}
...
.refresh-btn{display:none}
.modal{margin:6px!important;max-width:calc(100vw - 12px)!important}
.drawer{width:100vw!important;max-width:100vw!important}
.desk-only{display:none!important}

...
const TL:any[]=[];
type Tab='overview'|'alerts'|'coverage'|'vulns'|'intel'|'runbooks'|'incidents'|'compliance'|'tools';
...
/* ═══ AI CO-PILOT PANEL ═══ */
function AICopilot({alert,onClose,allAlerts}:{alert:Al|null;onClose:()=>void;allAlerts?:Al[]}){
...
function ExecSummary({metrics,alerts,coverage}:{metrics:any;alerts:Al[];coverage:any}){
  ...
  async function generate(){setOpen(true);setLoading(true);try{const r=await fetch('/api/exec-summary',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({metrics,alerts:alerts.slice(0,10),coverage,vulns:null})});const d=await r.json();setSummary(d.summary||d.error||'No summary generated')}catch(e){setSummary('Failed to generate summary. Check your Anthropic API key in the Tools tab.')}setLoading(false)}
  return<><button className="tc-btn" onClick={generate} style={{fontSize:'.66rem',padding:'3px 8px'}}>📋 Exec Summary</button><button className="tc-btn" onClick={()=>window.location.href='/report'} style={{fontSize:'.66rem',padding:'3px 8px'}}>📊 PDF Report</button><button className="acct-btn" onClick={()=>window.location.href='/embed'}>📡 Status Widget</button>{open&&<div className="modal-overlay" onClick={()=>setOpen(false)}><div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:640}}><div className="modal-hd"><h3>📋 Executive Security Summary</h3><button className="modal-close" onClick={()=>setOpen(false)}>✕</button></div><div className="modal-body">{loading?<div style={{padding:20,textAlign:'center'}}><span className="spin" style={{display:'inline-block'}}/> Generating...</div>:<div className="ai-response" style={{lineHeight:1.7}}>{summary.split('\n\n').map((p,i)=><p key={i} style={{marginBottom:12,fontSize:'.82rem'}}>{p}</p>)}</div>}</div><div className="modal-ft"><button className="tc-btn" onClick={()=>{const b=new Blob([summary],{type:'text/plain'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download='exec-summary.txt';a.click()}}>📥 Download</button><button className="tc-btn" onClick={()=>setOpen(false)}>Close</button></div></div></div>}</>;
}
...
function UserGuide({open,onClose}:{open:boolean;onClose:()=>void}){
  if(!open)return null;
  const sections=[
    {title:'Overview — War Room',icon:'◉',items:['Security Posture Score — animated 0-100 gauge calculated from real Tenable + Taegis data, graded A-F','Quiet Hours Indicator — time since last critical, time since last alert, active critical count (colour-coded)','Quick Actions Bar — IOC Search, New Taegis Investigation, Launch Tenable Scan, Shift Handover','Live Alert Stream — right sidebar scrolling real-time critical alerts, click any to analyse','KPI Cards — Alerts 24h, MTTR, MTTD, Open Incidents, Critical Vulns, Agent Coverage, Tools Active (with sparkline trends)','Severity Ring — donut breakdown with glow effects','Hourly Alert Chart — gradient bar chart of 24h alert volume','Connected Tools — which integrations are live','Top Risk Assets — top 8 devices by Tenable exposure score','Alert Sources — bar chart of which tools generate most alerts','Activity Timeline — chronological feed from real alerts, click for context','Natural Language Query — ask questions in plain English across all connected tools','Executive Summary — AI-generated board-ready security posture report']},
    {title:'Alerts Tab',icon:'⚡',items:['Unified alert feed from Taegis XDR + Defender (critical alerts only by default)','Auto-Triage — Claude AI assigns TP/FP/Suspicious with confidence + reasoning','AI Co-Pilot — click any alert for instant analysis with 3 tabs:','  → Analysis: Claude analyses the alert with follow-up questions','  → Runbook: auto-generated step-by-step response checklist per alert type (phishing, lateral movement, malware, etc.)','  → Related: shows all other alerts from the same device/user for context','Create Taegis Investigation — one-click from AI Co-Pilot, creates real investigation in Taegis XDR','Response Actions — Isolate/Restore device via Taegis, Block IP, Disable User, Run Scan','Send to Slack/Teams — push any alert to your SOC channel','Alert Notes — attach investigation notes to any alert (stored in Redis)','Alert Correlation — groups related alerts by device/user','Export to CSV']},
    {title:'Coverage Tab',icon:'🛡',items:['Total Assets with agent coverage percentage from real Tenable data','Stale Assets — devices not seen in 14+ days','Taegis Endpoints — click button to load all Taegis-managed endpoints with isolation status','OS Breakdown — real operating system distribution, legacy OS flagged red','Coverage Gaps table — hostname, IP, OS, missing agent, reason, last seen','Tool coverage per connected integration']},
    {title:'Vulnerabilities Tab',icon:'🔓',items:['Sub-views: Vulns | Hosts | Compliance | Scans','Patch Priority — top 20 vulns ranked by VPR × host count (actual risk score)','Vuln table — Plugin ID, Name, Severity, CVSS3, VPR, Hosts, Family, State (all from real Tenable)','Click any vuln → drawer with affected hosts, remediation steps, synopsis, CVE links, references','Risky Hosts — sorted by exposure score with ACR, agent status, last seen','Compliance Checks — pass/warn/fail with compliance score percentage','Scan Management — view all Tenable scans, launch scans directly from dashboard','Export: CSV download + Remediation Report (grouped by severity, sorted by VPR)','Severity filter pills + search by plugin name/ID/family']},
    {title:'Intel Tab',icon:'🛡',items:['Threat Intel Feed — industry-specific, powered by Claude web search, with source links','12 industry categories saved to Redis','Expandable cards with MITRE mappings + IOCs','Attack Chain — AI-generated kill chain graph','Predictions — Claude-generated SOC predictions with confidence + impact + actions','MITRE ATT&CK Heatmap — technique counts from real alerts','Trend Charts — sparklines for key metrics over time']},
    {title:'Tools Tab',icon:'🔌',items:['20 supported integrations: Defender, Taegis, CrowdStrike, SentinelOne, Tenable, Qualys, Rapid7, Zscaler ZIA/ZPA, Darktrace, Splunk, Sentinel, QRadar, and more','Add/edit credentials directly from the UI (stored in Upstash Redis)','System Health Check — tests all API connections, shows latency per endpoint','Toggle tools on/off, filter by category','Connection status with LIVE/DEMO/ERROR indicators']},
    {title:'TV Wall Mode',icon:'📺',items:['Click 📺 in topbar to enter fullscreen auto-rotating display','4 slides rotating every 15 seconds:','  → Security Posture Score + KPI counters','  → Live Alert List','  → Severity Distribution Ring + counters','  → Hourly Trend Chart + MTTR/MTTD','Slide indicators, live clock, click anywhere to exit','Designed for SOC wall monitors']},
    {title:'Critical Alert Popup',icon:'🚨',items:['When a new critical alert arrives, a red-bordered modal appears automatically','Shows alert title, source, device, MITRE technique','One-click: Analyse (AI Co-Pilot), View Alerts, or Dismiss','Pulsing red border animation']},
    {title:'Shift Handover',icon:'🔄',items:['Quick Actions → Shift Handover','Claude generates a summary from real Taegis + Tenable data (last 8 hours)','Shows 4-6 items with status (open/resolved/monitoring), title, detail, priority']},
    {title:'IOC Search',icon:'🔍',items:['Click 🔍 in topbar or press /','Searches real Tenable assets + vulnerabilities + Taegis alerts','Results grouped by tool with severity and match detail']},
    {title:'Account & Billing',icon:'👤',items:['Click your initial in the topbar to open account menu','Shows: email, organisation, plan, trial end date','Manage Billing — opens Stripe customer portal','Upgrade Plan — redirects to Stripe checkout','Sign Out — clears session and redirects to login']},
  ];
  const shortcuts=[['/','\'IOC Search'],['?','User Guide'],['f','Fullscreen'],['k','Command Palette (Cmd/Ctrl+K)'],['Esc','Close modals']];
  const setup=[
    {label:'1. Create Account',key:'Sign up at watchtower.vercel.app',desc:'Free Starter plan or 14-day Pro trial'},
    {label:'2. Upstash Redis',key:'UPSTASH_REDIS_REST_URL + TOKEN',desc:'Required — stores credentials, notes, settings, users'},
    {label:'3. Connect Tools',key:'Tools tab → Add credentials',desc:'Tenable, Taegis, Defender, Zscaler, etc.'},
    {label:'4. Anthropic API',key:'ANTHROPIC_API_KEY (via Tools tab)',desc:'Enables AI Co-Pilot, triage, predictions, threat intel, exec summary'},
    {label:'5. Slack/Teams',key:'SLACK_WEBHOOK_URL or TEAMS_WEBHOOK_URL',desc:'Optional — push alerts to a channel'},
    {label:'6. Stripe (SaaS)',key:'STRIPE_SECRET_KEY + price IDs',desc:'Optional — enables paid subscriptions and billing portal'},
  ];
  return(<div className="guide-overlay" onClick={onClose}><div className="guide-panel" onClick={e=>e.stopPropagation()}><div className="guide-hd"><div><h2 style={{fontSize:'1.05rem',fontWeight:800}}>📖 Watchtower User Guide</h2><p className="muted" style={{fontSize:'.7rem'}}>v26 — Single pane of glass for your SOC</p></div><button className="modal-close" onClick={onClose}>✕</button></div><div className="guide-body">{sections.map(s=>(<div key={s.title} className="guide-section"><h3><span style={{marginRight:6}}>{s.icon}</span>{s.title}</h3><ul>{s.items.map((item,i)=>(<li key={i} style={{paddingLeft:item.startsWith('  →')?16:0,color:item.startsWith('  →')?'var(--accent)':'var(--t2)'}}>{item}</li>))}</ul></div>))}<div className="guide-section"><h3>⌨️ Keyboard Shortcuts</h3><div className="shortcut-grid">{shortcuts.map(([key,desc])=>(<div key={key} className="shortcut-row"><kbd>{key}</kbd><span>{desc}</span></div>))}</div></div><div className="guide-section"><h3>⚙️ Setup Guide</h3><div style={{fontSize:'.76rem'}}>{setup.map((s,i)=>(<div key={s.label} style={{marginBottom:10}}><div style={{fontWeight:700,color:'var(--t1)'}}>{s.label}</div><div className="muted" style={{fontSize:'.68rem'}}><span className="mono" style={{color:'var(--accent)'}}>{s.key}</span> — {s.desc}</div></div>))}</div></div><div className="guide-section"><h3>📊 Pricing Tiers</h3><div style={{fontSize:'.76rem',color:'var(--t2)'}}><div style={{marginBottom:6}}><strong style={{color:'var(--t1)'}}>Starter (Free)</strong> — 2 tool integrations, dashboard, alerts, coverage view, manual refresh</div><div style={{marginBottom:6}}><strong style={{color:'var(--accent)'}}>Pro (£49/mo)</strong> — Unlimited tools, AI Co-Pilot, auto-triage, runbooks, threat intel, TV Wall, shift handover, 30s auto-refresh, priority support</div><div><strong style={{color:'var(--purple)'}}>Enterprise (£199/mo)</strong> — Everything in Pro + multi-tenant, RBAC, audit logs, SLA reporting, API access, SSO/SAML, dedicated support</div></div></div><div className="guide-section"><h3>🔒 Security & Privacy</h3><p style={{fontSize:'.76rem',color:'var(--t2)'}}>All credentials are stored in your Upstash Redis instance. Authentication uses httpOnly secure cookies with 30-day expiry. No data is sent to external services except when calling your connected tool APIs and (optionally) the Anthropic API for AI features. All processing happens in Vercel serverless functions. Audit logs track all logins, registrations, and subscription changes.</p></div><div className="guide-section"><h3>🔗 Connected Tool APIs</h3><p style={{fontSize:'.76rem',color:'var(--t2)'}}>Watchtower pulls real data from your security tools via their APIs. Currently confirmed working: Tenable.io (vulns, assets, compliance, scans), Secureworks Taegis XDR (alerts, investigations, endpoint isolation). Credentials are read from Redis first, then environment variables. All API calls are made server-side from Vercel functions — no credentials are ever exposed to the browser.</p></div></div></div></div>);
}
...
function AttackChainGraph(){
  const[data,setData]=useState<any>(null);const[loading,setLoading]=useState(false);const[selectedNode,setSelectedNode]=useState<any>(null);
  function reload(){setLoading(true);fetch('/api/attack-chain?t='+Date.now()).then(r=>r.ok?r.json():null).then(d=>{if(d)setData(d);setLoading(false)}).catch(()=>setLoading(false))}
  useEffect(()=>{fetch('/api/attack-chain?t='+Date.now()).then(r=>r.ok?r.json():null).then(d=>{if(d)setData(d)}).catch(()=>{})},[]);
  ...
  nodes.forEach((n:any)=>{const p=pos[n.id];if(!p)return;svg+=`<g class="chain-node" data-id="${n.id}" style="cursor:pointer"><circle cx="${p.x}" cy="${p.y}" r="18" fill="${(sc[n.sev]||'#171c28')+'20'}" stroke="${sc[n.sev]||'#1a2030'}" stroke-width="1.5"/><text x="${p.x}" y="${p.y+4}" text-anchor="middle" font-size="11" dominant-baseline="middle">${ti[n.type]||'?'}</text><text x="${p.x}" y="${p.y+28}" text-anchor="middle" fill="#e4e9f2" font-size="7" font-weight="600" font-family="monospace">${n.label.split('.')[0]}</text></g>`});
  return <div className="panel"><div className="panel-hd"><h3>🕸 Attack Chain</h3><span className="count">{nodes.length} nodes · {edges.length} edges</span><RefreshBtn onClick={reload} loading={loading}/></div><div style={{padding:8,overflowX:'auto'}}><svg width="620" height="280" style={{display:'block',margin:'0 auto'}} dangerouslySetInnerHTML={{__html:svg}} onClick={(e:any)=>{const g=e.target.closest('.chain-node');if(g){const nid=g.getAttribute('data-id');const node=nodes.find((n:any)=>n.id===nid);if(node)setSelectedNode(node)}}}/>{selectedNode&&<div style={{padding:10,background:'var(--bg2)',border:'1px solid var(--brd)',borderRadius:'var(--r)',marginTop:8}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{display:'flex',gap:6,alignItems:'center'}}><span style={{fontSize:'1.1rem'}}>{selectedNode.type==='device'?'🖥':selectedNode.type==='user'?'👤':'🌐'}</span><span style={{fontFamily:'var(--fm)',fontSize:'.82rem',fontWeight:700}}>{selectedNode.label}</span><span className={`sev sev-${selectedNode.sev}`}>{selectedNode.sev}</span></div><button className="modal-close" onClick={()=>setSelectedNode(null)} style={{fontSize:'.7rem'}}>✕</button></div><div style={{fontSize:'.74rem',color:'var(--t2)',marginTop:6,lineHeight:1.6}}>{selectedNode.id==='C2'?'Known C2 server at 185.220.101.42. Associated with Cobalt Strike infrastructure. Outbound beacons detected every 60 seconds. Blocked by Zscaler ZIA.':selectedNode.id==='WS042'?'Workstation compromised via phishing email. Encoded PowerShell execution detected. C2 beacon established. User credentials harvested via LSASS dump. Device isolated by SOC.':selectedNode.id==='jsmith'?'User account compromised via initial phishing. Credentials used for lateral movement to DC01. Account disabled pending investigation. Password reset required.':selectedNode.id==='DC01'?'Domain controller targeted for credential dumping via Mimikatz. Admin service account extracted. Kerberos golden ticket possible. Emergency: rotate krbtgt twice.':selectedNode.id==='admin_svc'?'Service account with domain admin privileges. Extracted from DC01 memory. Used for lateral SMB movement to file server. Recommend: disable, rotate, review SPNs.':selectedNode.id==='FS01'?'File server accessed via compromised admin_svc account over SMB. Checking for data exfiltration. DLP alerts triggered for 2.4GB outbound transfer attempt.':selectedNode.id==='DNS'?'DNS tunnelling detected on port 53. Encoded data exfiltration channel. Average query size 180 bytes vs normal 40 bytes. Destination: suspicious dynamic DNS provider.':selectedNode.id==='MAIL'?'Mail gateway identified as initial entry point. Phishing email with weaponised attachment delivered to jsmith. Attachment: Invoice_Q4.xlsm with macro dropper.':'Node in the attack chain. Review related alerts for details.'}</div><div style={{display:'flex',gap:3,marginTop:6,flexWrap:'wrap'}}>{edges.filter((e:any)=>e.from===selectedNode.id||e.to===selectedNode.id).map((e:any,i:number)=>(<span key={i} className="mitre">{e.mitre||e.label}</span>))}</div></div>}</div></div>;
}
...
function CollabBar(){
  ...
  if(!data)return <div className="panel"><div className="panel-hd"><h3>🔮 Predictions</h3></div><div style={{padding:16,textAlign:'center',color:'var(--t3)',fontSize:'.78rem'}}><span className="spin" style={{display:'inline-block'}}/></div></div>;
  if(data.noPredictions||!data.predictions||data.predictions.length===0)return <div className="panel"><div className="panel-hd"><h3>🔮 Predictions</h3></div><div style={{padding:16,textAlign:'center',color:'var(--t3)',fontSize:'.78rem'}}>{data.reason||'Configure Anthropic API key for AI predictions'}</div></div>;
...
function PredictionsPanel(){
  const[data,setData]=useState<any>(null);const[open,setOpen]=useState(false);const[loading,setLoading]=useState(false);
  function reload(){setLoading(true);fetch('/api/predictions?t='+Date.now()).then(r=>r.ok?r.json():null).then(d=>{if(d)setData(d);setLoading(false)}).catch(()=>setLoading(false))}
  useEffect(()=>{fetch('/api/predictions?t='+Date.now()).then(r=>r.ok?r.json():null).then(d=>{if(d)setData(d)}).catch(()=>{})},[]);
  if(!data)return <div className="panel"><div className="panel-hd"><h3>🔮 Predictive Analytics</h3><span className="count">...</span></div><div style={{padding:20,textAlign:'center',color:'var(--t3)'}}><span className="spin" style={{display:'inline-block'}}/></div></div>;
  return(<div className="panel"><div className="panel-hd"><h3>🔮 Predictive Analytics</h3><span className="count">{data.predictions.length}</span><RefreshBtn onClick={reload} loading={loading}/></div><div style={{padding:8}}>{data.predictions.map((p:any)=>(<div key={p.id} className="pred-card" style={{borderLeftColor:p.severity==='critical'?'var(--red)':p.severity==='high'?'#f97316':'var(--amber)'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}><div style={{flex:1}}><div style={{display:'flex',gap:4,alignItems:'center',marginBottom:2}}><span>{p.icon}</span><span className={`sev sev-${p.severity}`}>{p.severity}</span><span style={{fontSize:'.58rem',color:'var(--t3)',fontFamily:'var(--fm)',background:'var(--bg3)',padding:'1px 5px',borderRadius:3}}>{p.confidence}% conf</span></div><div style={{fontSize:'.78rem',fontWeight:600}}>{p.title}</div><div style={{fontSize:'.68rem',color:'var(--t2)',marginTop:2}}>{p.detail}{p.impact&&<div style={{fontSize:'.62rem',color:'var(--t3)',marginTop:2}}>Impact: {p.impact}</div>}{p.action&&<div style={{fontSize:'.62rem',color:'var(--green)',marginTop:2}}>Action: {p.action}</div>}</div></div><span style={{fontSize:'.6rem',color:'var(--t3)',fontFamily:'var(--fm)',whiteSpace:'nowrap',marginLeft:8}}>{p.timeframe}</span></div></div>))}</div></div>);
}
...
function DashboardInner(){
  ...
  const[userInfo,setUserInfo]=useState<any>(null);const[showAccount,setShowAccount]=useState(false);
  const[slaData,setSlaData]=useState<any>(null);const[incidents,setIncidents]=useState<any[]>([]);const[incidentModal,setIncidentModal]=useState<any>(null);const[customRunbooks,setCustomRunbooks]=useState<any[]>([]);const[compData,setCompData]=useState<any>(null);const[compFw,setCompFw]=useState('soc2');
  ...
  useEffect(()=>{fetch('/api/sla').then(r=>r.ok?r.json():null).then(d=>{if(d)setSlaData(d)}).catch(()=>{});fetch('/api/incidents').then(r=>r.ok?r.json():null).then(d=>{if(d?.incidents)setIncidents(d.incidents)}).catch(()=>{});fetch('/api/runbooks').then(r=>r.ok?r.json():null).then(d=>{if(d?.runbooks)setCustomRunbooks(d.runbooks)}).catch(()=>{});fetch('/api/auth/me').then(r=>r.ok?r.json():null).then(d=>{if(d?.user)setUserInfo(d)}).catch(()=>{})},[]);
  ...
  const tabs:{k:Tab;l:string;i:string;badge?:number}[]=[{k:'overview',l:'Overview',i:'◉'},{k:'alerts',l:'Alerts',i:'⚡',badge:critCount},{k:'coverage',l:'Coverage',i:'🛡'},{k:'vulns',l:'Vulns',i:'🔓'},{k:'intel',l:'Intel',i:'🛡'},{k:'incidents',l:'Incidents',i:'📁'},{k:'runbooks',l:'Runbooks',i:'📋'},{k:'compliance',l:'Compliance',i:'✅'},{k:'tools',l:`Tools (${enabledTools.length})`,i:'🔌'}];
...
/* ═══ COMPLIANCE MAP ═══ */
function ComplianceMap({data,framework,setFramework}:{data:any;framework:string;setFramework:(f:string)=>void}){
  useEffect(()=>{fetch('/api/compliance-map?framework='+framework).then(r=>r.ok?r.json():null).then(d=>{if(d&&setFramework)setCompData(d)}).catch(()=>{})},[framework]);
  const[compData,setCompData]=useState<any>(data);
  useEffect(()=>{if(!compData)fetch('/api/compliance-map?framework='+framework).then(r=>r.ok?r.json():null).then(d=>{if(d)setCompData(d)})},[]);
  const d=compData;
  if(!d)return <div style={{textAlign:'center',padding:40,color:'var(--t3)'}}>Loading compliance data...</div>;
  const statusColor:Record<string,string>={covered:'var(--green)',partial:'var(--amber)',gap:'var(--red)'};
  const statusLabel:Record<string,string>={covered:'Covered',partial:'Partial',gap:'Gap'};
  const cats=[...new Set(d.controls?.map((c:any)=>c.category)||[])];
  return <><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}><div><h2 style={{fontSize:'1rem',fontWeight:800}}>✅ Compliance Mapping</h2><p className="muted" style={{fontSize:'.72rem'}}>{d.name} — {d.overall}% coverage</p></div><div style={{display:'flex',gap:6}}>{(d.available||['soc2','iso27001']).map((fw:string)=>(<button key={fw} className={`tc-btn ${framework===fw?'tc-btn-primary':''}`} onClick={()=>{setFramework(fw);fetch('/api/compliance-map?framework='+fw).then(r=>r.ok?r.json():null).then(dd=>{if(dd)setCompData(dd)})}} style={{fontSize:'.66rem'}}>{fw==='soc2'?'SOC 2':'ISO 27001'}</button>))}</div></div><div className="kpi-grid" style={{gridTemplateColumns:'repeat(4,1fr)',marginBottom:12}}><div className="kpi"><div className="kpi-val" style={{color:'var(--accent)'}}>{d.overall}%</div><div className="kpi-label">Overall</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--green)'}}>{d.covered}</div><div className="kpi-label">Covered</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--amber)'}}>{d.partial}</div><div className="kpi-label">Partial</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--red)'}}>{d.gaps}</div><div className="kpi-label">Gaps</div></div></div>{cats.map(cat=>(<div key={cat} className="panel" style={{marginBottom:10}}><div className="panel-hd"><h3>{cat}</h3></div><div className="tbl-wrap"><table className="tbl"><thead><tr><th>Control</th><th>Name</th><th>Coverage</th><th>Status</th><th>Checks</th></tr></thead><tbody>{(d.controls||[]).filter((ctrl:any)=>ctrl.category===cat).map((ctrl:any)=>(<tr key={ctrl.id}><td className="mono" style={{fontWeight:700,fontSize:'.72rem',color:'var(--accent)'}}>{ctrl.id}</td><td style={{fontWeight:600,fontSize:'.76rem'}}>{ctrl.name}</td><td><div style={{display:'flex',alignItems:'center',gap:6}}><div style={{flex:1,height:6,background:'var(--bg3)',borderRadius:3,overflow:'hidden',minWidth:60}}><div style={{height:'100%',width:ctrl.coverage+'%',background:statusColor[ctrl.status],borderRadius:3}}/></div><span className="mono" style={{fontSize:'.66rem',minWidth:28}}>{ctrl.coverage}%</span></div></td><td><span style={{fontSize:'.58rem',fontWeight:700,color:statusColor[ctrl.status],background:statusColor[ctrl.status]+'15',padding:'2px 6px',borderRadius:4}}>{statusLabel[ctrl.status]}</span></td><td style={{fontSize:'.62rem',color:'var(--t3)'}}>{ctrl.featureChecks}/{ctrl.featureTotal}</td></tr>))}</tbody></table></div></div>))}</>;
}

/* ═══ RUNBOOK BUILDER ═══ */
function RunbookBuilder({runbooks,onUpdate}:{runbooks:any[];onUpdate:()=>void}){
  const[editing,setEditing]=useState<any>(null);const[title,setTitle]=useState('');const[keywords,setKeywords]=useState('');const[steps,setSteps]=useState<any[]>([]);const[category,setCategory]=useState('General');const[severity,setSeverity]=useState('all');
  function startNew(){setEditing({id:null});setTitle('New Runbook');setKeywords('');setSteps([{title:'',detail:'',cmd:null}]);setCategory('General');setSeverity('all')}
  function startEdit(rb:any){setEditing(rb);setTitle(rb.title);setKeywords((rb.triggerKeywords||[]).join(', '));setSteps(rb.steps||[{title:'',detail:'',cmd:null}]);setCategory(rb.category||'General');setSeverity(rb.severity||'all')}
  function addStep(){setSteps(p=>[...p,{title:'',detail:'',cmd:null}])}
  function removeStep(i:number){setSteps(p=>p.filter((_,idx)=>idx!==i))}
  function updateStep(i:number,field:string,val:string){setSteps(p=>p.map((s,idx)=>idx===i?{...s,[field]:val}:s))}
  async function save(){const body:any={action:editing.id?'update':'create',title,triggerKeywords:keywords.split(',').map((k:string)=>k.trim()).filter(Boolean),steps,category,severity};if(editing.id)body.id=editing.id;await fetch('/api/runbooks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});setEditing(null);onUpdate()}
  async function del(id:string){if(!confirm('Delete this runbook?'))return;await fetch('/api/runbooks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',id})});onUpdate()}
  async function dup(id:string){await fetch('/api/runbooks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'duplicate',id})});onUpdate()}
  const templates=[{name:'Phishing Response',cat:'Email',kw:'phish,credential harvest,email',steps:[{title:'Quarantine the email',detail:'Remove from all mailboxes, block sender domain',cmd:'Search-Mailbox -SearchQuery "from:sender" -DeleteContent'},{title:'Reset affected credentials',detail:'Force password reset for any user who clicked',cmd:null},{title:'Check auth logs',detail:'Review sign-in logs for 24h — impossible travel, new devices',cmd:null},{title:'Block IOCs',detail:'Add sender IP, domain, payload hashes to blocklists',cmd:null},{title:'Notify affected users',detail:'Targeted awareness notification to all recipients',cmd:null}]},{name:'Ransomware Response',cat:'Malware',kw:'ransomware,encrypt,ransom',steps:[{title:'Isolate affected devices',detail:'Network-isolate immediately to prevent spread',cmd:null},{title:'Identify strain',detail:'Check hash on VirusTotal, identify ransomware family',cmd:null},{title:'Check backup integrity',detail:'Verify backups are clean and accessible',cmd:null},{title:'Engage IR team',detail:'Escalate to incident response, consider legal/PR',cmd:null},{title:'Preserve evidence',detail:'Forensic image before any remediation',cmd:null}]},{name:'Lateral Movement',cat:'Network',kw:'lateral,smb,rdp,movement',steps:[{title:'Map the path',detail:'Identify source account, origin device, target device',cmd:null},{title:'Disable compromised account',detail:'Immediately disable the account used',cmd:null},{title:'Isolate target',detail:'Network-isolate target to prevent further spread',cmd:null},{title:'Audit data access',detail:'Review file access logs on target for exfiltration',cmd:null},{title:'Rotate service accounts',detail:'If service account was used, rotate all credentials',cmd:null}]}];
  return <><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,flexWrap:'wrap',gap:8}}><h2 style={{fontSize:'1rem',fontWeight:800}}>📋 Runbooks ({runbooks.length})</h2><div style={{display:'flex',gap:6}}><button className="tc-btn tc-btn-primary" onClick={startNew} style={{fontSize:'.68rem'}}>+ New Runbook</button></div></div>{!editing&&runbooks.length===0&&<div className="panel" style={{padding:24}}><div style={{textAlign:'center',marginBottom:16}}><div style={{fontSize:'1.8rem',opacity:.3,marginBottom:8}}>📋</div><div style={{fontWeight:700,fontSize:'.88rem'}}>No custom runbooks yet</div><div className="muted" style={{fontSize:'.72rem',marginTop:4}}>Create your own or start from a template</div></div><div style={{display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap'}}>{templates.map(t=>(<button key={t.name} className="tc-btn" onClick={()=>{setEditing({id:null});setTitle(t.name);setKeywords(t.kw);setSteps(t.steps);setCategory(t.cat);setSeverity('all')}} style={{fontSize:'.68rem'}}>{t.name}</button>))}</div></div>}{!editing&&runbooks.length>0&&<div className="panel"><div className="tbl-wrap"><table className="tbl"><thead><tr><th>Runbook</th><th>Category</th><th>Triggers</th><th>Steps</th><th>Actions</th></tr></thead><tbody>{runbooks.map(rb=>(<tr key={rb.id}><td style={{fontWeight:600}}>{rb.title}</td><td style={{fontSize:'.7rem'}}>{rb.category}</td><td style={{fontSize:'.65rem',color:'var(--t3)'}}>{(rb.triggerKeywords||[]).join(', ')}</td><td className="mono">{rb.steps?.length||0}</td><td style={{display:'flex',gap:4}}><button className="tc-btn" onClick={()=>startEdit(rb)} style={{fontSize:'.58rem',padding:'2px 6px'}}>Edit</button><button className="tc-btn" onClick={()=>dup(rb.id)} style={{fontSize:'.58rem',padding:'2px 6px'}}>Clone</button><button className="tc-btn tc-btn-danger" onClick={()=>del(rb.id)} style={{fontSize:'.58rem',padding:'2px 6px'}}>Delete</button></td></tr>))}</tbody></table></div></div>}{editing&&<div className="panel" style={{padding:16}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}><h3 style={{fontSize:'.88rem',fontWeight:800}}>{editing.id?'Edit Runbook':'New Runbook'}</h3><button className="modal-close" onClick={()=>setEditing(null)}>✕</button></div><div style={{display:'flex',gap:8,marginBottom:10,flexWrap:'wrap'}}><input className="field-input" placeholder="Runbook title" value={title} onChange={e=>setTitle(e.target.value)} style={{flex:2,minWidth:200,fontSize:'.78rem',padding:'6px 10px'}}/><select className="field-input" value={category} onChange={e=>setCategory(e.target.value)} style={{fontSize:'.72rem',padding:'6px 8px'}}><option value="General">General</option><option value="Email">Email</option><option value="Malware">Malware</option><option value="Network">Network</option><option value="Identity">Identity</option><option value="Cloud">Cloud</option></select><select className="field-input" value={severity} onChange={e=>setSeverity(e.target.value)} style={{fontSize:'.72rem',padding:'6px 8px'}}><option value="all">All severities</option><option value="critical">Critical only</option><option value="high">High+</option></select></div><div style={{marginBottom:10}}><label style={{fontSize:'.68rem',fontWeight:600,color:'var(--t3)',display:'block',marginBottom:3}}>Trigger keywords (comma-separated)</label><input className="field-input" placeholder="phish, credential, email" value={keywords} onChange={e=>setKeywords(e.target.value)} style={{width:'100%',fontSize:'.72rem',padding:'6px 10px'}}/></div><div style={{marginBottom:10}}><label style={{fontSize:'.68rem',fontWeight:600,color:'var(--t3)',display:'block',marginBottom:6}}>Steps</label>{steps.map((s,i)=>(<div key={i} style={{display:'flex',gap:6,marginBottom:6,alignItems:'flex-start'}}><div className="rb-num" style={{marginTop:2}}>{i+1}</div><div style={{flex:1}}><input className="field-input" placeholder="Step title" value={s.title} onChange={e=>updateStep(i,'title',e.target.value)} style={{width:'100%',fontSize:'.72rem',padding:'5px 8px',marginBottom:3}}/><input className="field-input" placeholder="Details / instructions" value={s.detail} onChange={e=>updateStep(i,'detail',e.target.value)} style={{width:'100%',fontSize:'.68rem',padding:'4px 8px',marginBottom:2}}/><input className="field-input" placeholder="Command (optional)" value={s.cmd||''} onChange={e=>updateStep(i,'cmd',e.target.value||null)} style={{width:'100%',fontSize:'.62rem',padding:'3px 8px',fontFamily:'var(--fm)'}}/></div><button className="modal-close" onClick={()=>removeStep(i)} style={{fontSize:'.6rem',marginTop:4}}>✕</button></div>))}<button className="tc-btn" onClick={addStep} style={{fontSize:'.62rem'}}>+ Add Step</button></div><div style={{display:'flex',gap:6,justifyContent:'space-between'}}><div style={{display:'flex',gap:4}}>{templates.map(t=>(<button key={t.name} className="tc-btn" onClick={()=>{setTitle(t.name);setKeywords(t.kw);setSteps(t.steps);setCategory(t.cat)}} style={{fontSize:'.55rem',padding:'2px 5px'}}>📄 {t.name}</button>))}</div><button className="tc-btn tc-btn-primary" onClick={save} style={{fontSize:'.72rem'}}>💾 Save Runbook</button></div></div>}</>;
}
...
function Als({alerts,onAskAI,onDeviceDrill,onNotes}:{alerts:Al[];onAskAI?:(a:Al)=>void;onDeviceDrill?:(h:string)=>void;onNotes?:(id:string)=>void}){
  ...
  useEffect(()=>{if(triageOn&&alerts.length>0){fetch('/api/auto-triage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({alerts})}).then(r=>r.ok?r.json():{alerts:[]}).then(d=>setTriaged(d.alerts||[])).catch(()=>{})}},[triageOn,alerts]);
...
function ToolsManager({toolsData,onRefresh}:{toolsData:any;onRefresh:()=>void}){
  const[setupData,setSetupData]=useState<any>(null);const[redisUrl,setRedisUrl]=useState('');const[redisToken,setRedisToken]=useState('');const[redisTesting,setRedisTesting]=useState(false);const[redisResult,setRedisResult]=useState<any>(null);
  useEffect(()=>{fetch('/api/setup').then(r=>r.ok?r.json():null).then(d=>{if(d)setSetupData(d)}).catch(()=>{})},[]);
  ...
  async function migrateTools(){setMigrating(true);try{await fetch('/api/admin/migrate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({})});setTimeout(onRefresh,500)}catch(e){}setMigrating(false)}
...
:root,[data-theme="dark"]{--bg0:#05070c;--bg1:#0a0d15;--bg2:#0f1219;--bg3:#161a24;--bg4:#1c2130;--brd:#141928;--brd2:#1e2840;--t1:#eaf0ff;--t2:#8896b8;--t3:#4a5672;--t4:#2e3a52;--accent:#5b9aff;--accent2:#8b6fff;--accent-s:#5b9aff10;--red:#ff4466;--amber:#ffb340;--green:#34e8a5;--blue:#5b9aff;--purple:#a87cff;--reds:#ff446610;--ambers:#ffb34010;--greens:#34e8a510;--blues:#5b9aff10;--fm:'JetBrains Mono',monospace;--fs:'DM Sans',sans-serif;--r:10px;--r2:14px;--shadow:0 2px 8px rgba(0,0,0,.4),0 8px 32px rgba(0,0,0,.2);--glow:0 0 30px rgba(91,154,255,.08)}
[data-theme="light"]{--bg0:#f6f8fc;--bg1:#fff;--bg2:#f8fafd;--bg3:#eef2f9;--bg4:#e2e8f4;--brd:#d8e0ed;--brd2:#c0c9db;--t1:#0f1729;--t2:#475570;--t3:#8d99b0;--t4:#c6cdd8;--accent:#3b7ff5;--accent2:#6d28d9;--accent-s:#3b7ff508;--red:#e5293e;--amber:#d97706;--green:#0d9d6c;--blue:#3b7ff5;--purple:#7c3aed;--reds:#e5293e08;--ambers:#d9770608;--greens:#0d9d6c08;--blues:#3b7ff508;--shadow:0 1px 4px rgba(0,0,0,.06),0 6px 20px rgba(0,0,0,.04);--glow:none}

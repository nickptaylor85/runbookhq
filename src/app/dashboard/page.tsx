...
function DashboardInner(){
  ...
  const[slaData,setSlaData]=useState<any>(null);const[incidents,setIncidents]=useState<any[]>([]);const[incidentModal,setIncidentModal]=useState<any>(null);const[customRunbooks,setCustomRunbooks]=useState<any[]>([]);const[compData,setCompData]=useState<any>(null);const[compFw,setCompFw]=useState('soc2');const[liveIntel,setLiveIntel]=useState<any>(null);
...
function ComplianceMap({data,framework,setFramework}:{data:any;framework:string;setFramework:(f:string)=>void}){
  ...
  useEffect(()=>{setLoading(true);fetch('/api/compliance-map?framework='+framework).then(r=>r.ok?r.json():null).then(dd=>{if(dd)setD(dd);setLoading(false)}).catch(()=>setLoading(false))},[framework]);
  if(loading||!d)return <div style={{textAlign:'center',padding:40,color:'var(--t3)'}}>Loading compliance data...</div>;
  const statusIcon:Record<string,string>={automated:'✅',supported:'🟡',manual:'🔴'};
  const cats=[...new Set(d.controls?.map((c:any)=>c.category)||[])];
  return <><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}><div><h2 style={{fontSize:'1rem',fontWeight:800}}>✅ Compliance Mapping</h2><p className="muted" style={{fontSize:'.72rem'}}>{d.name} — {d.coveragePct}% automated coverage</p></div><div style={{display:'flex',gap:6}}>{(d.available||['soc2','iso27001']).map((fw:string)=>(<button key={fw} className={'tc-btn '+(framework===fw?'tc-btn-primary':'')} onClick={()=>setFramework(fw)} style={{fontSize:'.66rem'}}>{fw==='soc2'?'SOC 2':'ISO 27001'}</button>))}</div></div><div className="kpi-grid" style={{gridTemplateColumns:'repeat(4,1fr)',marginBottom:12}}><div className="kpi"><div className="kpi-val" style={{color:'var(--accent)'}}>{d.coveragePct}%</div><div className="kpi-label">Coverage</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--green)'}}>{d.automated}</div><div className="kpi-label">Automated</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--amber)'}}>{d.supported}</div><div className="kpi-label">Supported</div></div><div className="kpi"><div className="kpi-val" style={{color:'var(--red)'}}>{d.manual}</div><div className="kpi-label">Manual</div></div></div>{cats.map(cat=>(<div key={cat} className="panel" style={{marginBottom:10}}><div className="panel-hd"><h3>{cat}</h3></div><div style={{padding:'8px 14px'}}>{(d.controls||[]).filter((ctrl:any)=>ctrl.category===cat).map((ctrl:any)=>(<div key={ctrl.id} style={{display:'flex',gap:10,padding:'10px 0',borderBottom:'1px solid var(--brd)',alignItems:'flex-start'}}><span style={{fontSize:'1rem',flexShrink:0}}>{statusIcon[ctrl.status]||'❓'}</span><div style={{flex:1}}><div style={{display:'flex',gap:6,alignItems:'center',marginBottom:3}}><span className="mono" style={{fontSize:'.72rem',fontWeight:700,color:'var(--accent)'}}>{ctrl.id}</span><span style={{fontSize:'.78rem',fontWeight:600}}>{ctrl.name}</span></div><div style={{fontSize:'.72rem',color:'var(--t2)',lineHeight:1.5}}>{ctrl.watchtowerFeature}</div><div style={{fontSize:'.6rem',color:ctrl.statusColor==='green'?'var(--green)':ctrl.statusColor==='amber'?'var(--amber)':'var(--red)',fontWeight:600,marginTop:4}}>{ctrl.statusLabel}</div></div></div>))}</div></div>))}</>;
}

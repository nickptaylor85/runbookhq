...
function AICopilot({alert,onClose,allAlerts,customRunbooks}:{alert:Al|null;onClose:()=>void;allAlerts?:Al[];customRunbooks?:any[]}){
  ...
  async function ask(q?:string){setLoading(true);try{const r=await fetch('/api/ai-copilot',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({alert,question:q||question||undefined})});const d=await r.json();setResponse(d.response||d.analysis||'No response — check your Anthropic API key in Tools');setQuestion('')}catch(e){setResponse('Failed to get response. Check network connection.')}setLoading(false)}

...
function DashboardInner(){
  ...
  const[slaData,setSlaData]=useState<any>(null);const[incidents,setIncidents]=useState<any[]>([]);const[incidentModal,setIncidentModal]=useState<any>(null);
  ...
  useEffect(()=>{fetch('/api/sla').then(r=>r.ok?r.json():null).then(d=>{if(d)setSlaData(d)}).catch(()=>{});fetch('/api/incidents').then(r=>r.ok?r.json():null).then(d=>{if(d?.incidents)setIncidents(d.incidents)}).catch(()=>{});fetch('/api/auth/me').then(r=>r.ok?r.json():null).then(d=>{if(d?.user)setUserInfo(d)}).catch(()=>{})},[]);
  ...
  const visibleTabs=tabs.filter(t=>t.k==='tools'||t.k==='incidents'?userRole==='admin'||userRole==='superadmin':true);

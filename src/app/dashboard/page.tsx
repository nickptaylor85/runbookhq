...
function IntelTab({alerts,onAskAI,onDeviceDrill,liveIntel}:{alerts:Al[];onAskAI?:(a:Al)=>void;onDeviceDrill?:(h:string)=>void;liveIntel?:any}){
  return <><AIInsights tab="intel"/>{liveIntel&&<LiveIntelFeed data={liveIntel}/>}<ThreatIntelFeed/></>;
}
...
function Ov({m,cov,alerts,zsc,sparks,enabledTools,onAskAI,onRefresh,setTlDetail,onQuickAction,slaData,aiStats}:any){
  return <><div className="ai-brief"><div className="ai-brief-hd"><span className="ai-brief-dot"/>AI Shift Brief</div><div className="ai-brief-body">{m?.alertsLast24h?<><div className="ai-brief-text">Processed <strong>{m.alertsLast24h.total||0} alerts</strong> in the last 24 hours. AI auto-closed <strong>{aiStats?.autoClosed||0} false positives</strong> and escalated <strong>{aiStats?.escalated||0} true positives</strong> to incidents. {(m.alertsLast24h.critical||0)>0&&<span style={{color:'var(--red)'}}>There {m.alertsLast24h.critical===1?'is':'are'} <strong>{m.alertsLast24h.critical} critical alert{m.alertsLast24h.critical>1?'s':''}</strong> requiring attention.</span>} Your posture score is <strong>{cov?.score||'--'}</strong>.</div></>:<div className="ai-brief-text">Connecting to your security tools...</div>}</div></div><div className="war-room"><div className="wr-left"><div style={{display:'flex',gap:10,alignItems:'flex-start',marginBottom:10,flexWrap:'wrap'}}><PostureGauge/><div style={{flex:1,minWidth:200}}><KpiSection m={m} cov={cov} zsc={zsc} sparks={sparks} enabledTools={enabledTools} alerts={alerts}/></div></div><SLABar sla={slaData}/><QuickActions onAction={onQuickAction}/><AIStatsCard/></div><div className="wr-right"><AlertStream alerts={alerts} onAlert={onAskAI}/></div></div><OvCharts m={m} sparks={sparks} enabledTools={enabledTools}/></>;
}
...
function Als({alerts,onAskAI,onDeviceDrill,onNotes}:{alerts:Al[];onAskAI?:(a:Al)=>void;onDeviceDrill?:(h:string)=>void;onNotes?:(id:string)=>void}){
  ...
  return(<><AIInsights tab="alerts"/><div className="filter-row"><div className="pills">{['all','critical','high','medium','low'].map(s=>(<button key={s} className={`pill ${sev===s?'on':''}`} onClick={()=>setSev(s)}>{s==='all'?`All (${alerts.length})`:`${s.charAt(0).toUpperCase()+s.slice(1)} (${alerts.filter(a=>a.severity===s).length})`}</button>))}</div><button className={`tc-btn ${grouped?'tc-btn-primary':''}`} onClick={()=>setGrouped(!grouped)} style={{fontSize:'.66rem',padding:'3px 8px'}}>{grouped?'🔗 Correlated':'🔗 Correlate'}</button><ExportBtn data={()=>f.map(a=>({title:a.title,source:a.source,severity:a.severity,status:a.status,device:a.device,user:a.user,mitre:a.mitre,time:a.timestamp}))} filename="secops-alerts"/>{triageStats&&<span style={{fontSize:'.58rem',color:'var(--accent)',fontWeight:600,background:'var(--accent-s)',padding:'2px 8px',borderRadius:4,display:'inline-flex',alignItems:'center',gap:4}}><span style={{width:5,height:5,borderRadius:'50%',background:'var(--green)',boxShadow:'0 0 6px var(--green)'}}/>AI Triaged: {triageStats.tp} TP · {triageStats.fp} FP · {triageStats.suspicious} SUS · {triageStats.autoClosed} auto-closed</span>}<button className="tc-btn" onClick={()=>window.location.reload()} style={{fontSize:'.66rem',padding:'3px 8px'}}>↻ Refresh</button>{dismissed.size>0&&<button className="tc-btn" onClick={()=>setDismissed(new Set())} style={{fontSize:'.58rem',color:'var(--amber)'}}>{dismissed.size} dismissed — Undo</button>}</div><div className="panel"><div className="tbl-wrap" style={{maxHeight:'calc(100vh - 170px)'}}><table className="tbl"><thead><tr><th>Alert</th><th>Source</th><th>Sev</th><th className="desk-only">Status</th><th className="desk-only">Device</th><th className="desk-only">MITRE</th><th>Time</th><th className="desk-only">Actions</th></tr></thead><tbody>{(()=>{if(!grouped)return f.map(a=>(<tr key={a.id}><td style={{fontWeight:600,maxWidth:280}}><div style={{display:'flex',alignItems:'center',gap:4}}><span>{a.title}</span>{triageMap.has(a.id)&&<span style={{fontSize:'.5rem',fontWeight:800,padding:'1px 5px',borderRadius:3,flexShrink:0,color:triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'var(--red)':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'var(--green)':'var(--amber)',background:triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'var(--reds)':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'var(--greens)':'var(--ambers)'}}>{triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'TP':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'FP':'SUS'} {triageMap.get(a.id).confidence||''}%</span>}</div></td><td><span className={`src ${sc(a.source)}`}>{a.source}</span></td><td><span className={`sev sev-${a.severity}`}>{a.severity}</span></td><td className="desk-only"><span className={`status status-${a.status}`}>{a.status}</span></td><td className="device desk-only">{a.device?<span style={{cursor:"pointer",textDecoration:"underline dotted",textUnderlineOffset:2}} onClick={()=>onDeviceDrill?.(a.device)}>{a.device}</span>:"—"}</td><td className="desk-only">{a.mitre?<span className="mitre">{a.mitre}</span>:<span className="muted">—</span>}</td><td className="ts">{ago(a.timestamp)}</td><td className="desk-only"><ActionMenu alert={a} onDone={()=>{}}/><button className="tc-btn" onClick={()=>{const p=(window as any).__wt_plan||'community';const r2=(window as any).__wt_role||'';if(p==='community'&&r2!=='superadmin'){alert('AI Co-Pilot requires Team plan or higher');return}onAskAI?.(a)}} style={{fontSize:'.58rem',padding:'2px 5px'}}>🤖</button><button className="tc-btn" onClick={()=>onNotes?.(a.id)} style={{fontSize:'.58rem',padding:'2px 5px'}}>📝</button><button className="tc-btn" onClick={()=>fetch('/api/slack-webhook',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'send',alert:a})}).then(()=>{})} style={{fontSize:'.58rem',padding:'2px 5px'}} title="Send to Slack">💬</button><button className="tc-btn" onClick={()=>dismiss(a.id)} style={{fontSize:'.58rem',padding:'2px 5px'}} title="Dismiss">✕</button></td></tr>));
        ...
        return Object.entries(groups).flatMap(([key,als])=>{
          if(als.length===1)return als.map(a=>(<tr key={a.id}><td style={{fontWeight:600,maxWidth:280}}><div style={{display:'flex',alignItems:'center',gap:4}}><span>{a.title}</span>{triageMap.has(a.id)&&<span style={{fontSize:'.5rem',fontWeight:800,padding:'1px 5px',borderRadius:3,flexShrink:0,color:triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'var(--red)':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'var(--green)':'var(--amber)',background:triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'var(--reds)':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'var(--greens)':'var(--ambers)'}}>{triageMap.get(a.id).verdict==='tp'||triageMap.get(a.id).verdict==='true_positive'?'TP':triageMap.get(a.id).verdict==='fp'||triageMap.get(a.id).verdict==='false_positive'?'FP':'SUS'} {triageMap.get(a.id).confidence||''}%</span>}</div></td><td><span className={`src ${sc(a.source)}`}>{a.source}</span></td><td><span className={`sev sev-${a.severity}`}>{a.severity}</span></td><td className="desk-only"><span className={`status status-${a.status}`}>{a.status}</span></td><td className="device desk-only">{a.device?<span style={{cursor:"pointer",textDecoration:"underline dotted",textUnderlineOffset:2}} onClick={()=>onDeviceDrill?.(a.device)}>{a.device}</span>:"—"}</td><td className="desk-only">{a.mitre?<span className="mitre">{a.mitre}</span>:<span className="muted">—</span>}</td><td className="ts">{ago(a.timestamp)}</td><td className="desk-only"><ActionMenu alert={a} onDone={()=>{}}/><button className="tc-btn" onClick={()=>{const p=(window as any).__wt_plan||'community';const r2=(window as any).__wt_role||'';if(p==='community'&&r2!=='superadmin'){alert('AI Co-Pilot requires Team plan or higher');return}onAskAI?.(a)}} style={{fontSize:'.58rem',padding:'2px 5px'}}>🤖</button><button className="tc-btn" onClick={()=>onNotes?.(a.id)} style={{fontSize:'.58rem',padding:'2px 5px'}}>📝</button><button className="tc-btn" onClick={()=>fetch('/api/slack-webhook',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'send',alert:a})}).then(()=>{})} style={{fontSize:'.58rem',padding:'2px 5px'}} title="Send to Slack">💬</button><button className="tc-btn" onClick={()=>dismiss(a.id)} style={{fontSize:'.58rem',padding:'2px 5px'}} title="Dismiss">✕</button></td></tr>));

...
function Ov({m,cov,alerts,zsc,sparks,enabledTools,onAskAI,onRefresh,setTlDetail,onQuickAction,slaData,aiStats}:any){
  return <><AutomationSlider/><div className="ai-brief"><div className="ai-brief-hd"><span className="ai-brief-dot"/>AI Shift Brief</div><div className="ai-brief-body">{m?.alertsLast24h?<><div className="ai-brief-text">Processed <strong>{m.alertsLast24h.total||0} alerts</strong> in the last 24 hours. AI auto-closed <strong>{aiStats?.autoClosed||0} false positives</strong> and escalated <strong>{aiStats?.escalated||0} true positives</strong> to incidents. {(m.alertsLast24h.critical||0)>0&&<span style={{color:'var(--red)'}}>There {m.alertsLast24h.critical===1?'is':'are'} <strong>{m.alertsLast24h.critical} critical alert{m.alertsLast24h.critical>1?'s':''}</strong> requiring attention.</span>} Your posture score is <strong>{cov?.score||'--'}</strong>.</div></>:<div className="ai-brief-text">Connecting to your security tools...</div>}</div></div><div className="war-room"><div className="wr-left"><div style={{display:'flex',gap:10,alignItems:'flex-start',marginBottom:10,flexWrap:'wrap'}}><PostureGauge/><div style={{flex:1,minWidth:200}}><KpiSection m={m} cov={cov} zsc={zsc} sparks={sparks} enabledTools={enabledTools} alerts={alerts}/></div></div><SLABar sla={slaData}/><QuickActions onAction={onQuickAction}/><AIStatsCard/></div><div className="wr-right"><AlertStream alerts={alerts} onAlert={onAskAI}/></div></div><OvCharts m={m} sparks={sparks} enabledTools={enabledTools}/></>;
}

...
function AddonUsage(){
  ...
  return <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>{allAddons.map(a=>{const on=active.includes(a.id);const u=usage[a.id]||{};const metric=a.id==='ai'?(u.triagesThisMonth||0)+' triages':a.id==='intel'?(u.feedsActive||0)+' feeds':a.id==='reports'?(u.generatedThisMonth||0)+' reports':a.id==='api'?(u.callsThisMonth||0)+' API calls':a.id==='branding'?(u.enabled?'Active':'Inactive'):a.id==='support'?(u.ticketsOpen||0)+' tickets':'';return <div key={a.id} style={{padding:10,background:on?'var(--accent-s)':'var(--bg2)',border:'1px solid '+(on?'var(--accent)':'var(--brd)'),borderRadius:8,textAlign:'center'}}><div style={{fontSize:'1.1rem',marginBottom:4}}>{a.icon}</div><div style={{fontSize:'.72rem',fontWeight:700,color:on?'var(--t1)':'var(--t3)'}}>{a.name}</div><div style={{fontSize:'.58rem',color:on?'var(--accent)':'var(--t3)',fontFamily:'var(--fm)',marginTop:2}}>{on?metric:'Not active'}</div><div style={{fontSize:'.52rem',color:'var(--t3)',marginTop:3}}>{a.price}</div>{!on&&<button className="tc-btn" onClick={()=>window.location.href='/pricing'} style={{fontSize:'.52rem',padding:'2px 6px',marginTop:4}}>Activate</button>}</div>})}</div>;
}
...
.topbar-right .search-trigger{border:none;background:var(--bg2);border-radius:8px;padding:6px 12px;font-size:.72rem;color:var(--t3);cursor:pointer;display:flex;align-items:center;gap:4px;font-family:var(--fs);transition:all .15s;outline:none}
.search-trigger-old{padding:4px 6px;font-size:.72rem}

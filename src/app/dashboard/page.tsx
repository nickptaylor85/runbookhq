...
function AIInsights({tab}:{tab:string}){
  ...
  return <div className="ai-insights"><div className="ai-insights-hd"><span className="ai-insights-dot"/>AI Insights</div>{insights.map(i=>(<div key={i.id} className="ai-insight" style={{borderLeftColor:prioCol[i.priority]||'var(--accent)'}}><div className="ai-insight-top"><span className="ai-insight-icon">{i.icon}</span><span className="ai-insight-title">{i.title}</span></div><div className="ai-insight-detail">{i.detail}</div>{i.action&&<button className="tc-btn" onClick={()=>{if(i.actionTab){const e=new CustomEvent('wt-tab',{detail:i.actionTab});window.dispatchEvent(e)}}} style={{fontSize:'.62rem',padding:'3px 8px',marginTop:4}}>{i.action}</button>}</div>))}</div>;
}
...
.mnav-btn{flex:1;min-width:0;padding:6px 2px;font-size:.62rem;text-align:center;border:none;background:var(--bg2);color:var(--t2);border-radius:6px;cursor:pointer;font-family:var(--fs)}
.mnav-btn.active{background:var(--accent-s);color:var(--accent)}
.mob-menu{width:32px;height:32px;border:1px solid var(--brd);border-radius:6px;background:none;color:var(--t2);cursor:pointer;font-size:.9rem;display:flex;align-items:center;justify-content:center}
...
.desk-only{display:none!important}
.mob-only{display:flex!important}
.mob-nav{display:flex;flex-wrap:wrap;gap:2px;padding:6px 10px;background:var(--bg1);border-bottom:1px solid var(--brd)}
.mnav-btn{flex:1;min-width:0;padding:8px 4px;font-size:.68rem;text-align:center;border:none;background:var(--bg2);color:var(--t2);border-radius:6px;cursor:pointer;font-family:var(--fs)}
.mnav-btn.active{background:var(--accent-s);color:var(--accent)}
.mob-menu{display:flex;align-items:center;justify-content:center;width:32px;height:32px;border:1px solid var(--brd);border-radius:6px;background:none;color:var(--t2);cursor:pointer;font-size:1rem}

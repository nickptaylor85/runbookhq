...
export async function POST(req: Request) {
  ...
  if(action==='test'){
    ...
    try{await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:'🔔 Watchtower connected!'})});return NextResponse.json({ok:true})}catch(e){return NextResponse.json({error:'Webhook failed'},{status:500})}
  }

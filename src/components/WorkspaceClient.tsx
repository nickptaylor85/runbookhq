...
export default function WorkspaceClient({ user, savedData }: Props) {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#060a14]">
      {/* Top Bar */}
      <div className="h-11 flex-shrink-0 bg-[#0b1020] border-b border-[#1a2650] flex items-center justify-between px-2 sm:px-4 z-50">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="text-sm font-semibold text-white hidden sm:block">SecOpsHQ</span>
          </div>
          <div className="h-4 w-px bg-[#1a2650] mx-1 hidden sm:block"/>
          <div className="hidden md:flex items-center gap-1.5 text-xs text-[#4a5578]">
          ...
          {lastSaved && !saving && <span className="text-xs text-[#2d3756] font-mono hidden sm:block">Saved {lastSaved}</span>}
        </div>
      ...
      <iframe ref={iframeRef} src="/app.html" className="flex-1 w-full border-none" onLoad={handleIframeLoad} title="SecOpsHQ" sandbox="allow-scripts allow-same-origin allow-downloads allow-popups allow-modals" />
    </div>
  );
}
